							...
							if($ticketResult){
								...
								if($ticketResult->num_rows>0){
									...
									while ($ticketRows = mysqli_fetch_array($ticketResult,MYSQLI_ASSOC)) {
										...
										$objWorkSheet->getStyleByColumnAndRow($avariaCol+5,2)->applyFromArray($style);
										...
										$objWorkSheet->setCellValueByColumnAndRow($avariaCol+5,2, 'Reason')->getColumnDimensionByColumn($avariaCol+5,2)->setAutoSize(true);
										/************* Set Values **************/
										...
										if(!$parentCharged){
											...
											if($ticketRows['close_date']!=''){
										        ...
										        if(isset($_GET['fromDate']) && isset($_GET['toDate'])){
													...
													$fromDt = strtotime($_GET['fromDate']);
										...
										$reasonArr = CMDBSource::QueryToArray("SELECT rs.reason FROM ntticket tk LEFT JOIN ntreason rs ON rs.reason_id=tk.reason_id WHERE tk.id=".$ticketRows['ticket_id']);
										$reasonName = "";
										if(!empty($reasonArr)){
											$reasonName = $reasonArr[0]['reason'];
										}
										$objWorkSheet->setCellValueByColumnAndRow($avariaCol+5,$site_row, $reasonName);

										$avariaCol = $avariaCol+6;
