...
try
{
	...
	switch ($operation)
	{
		...
		case 'on_form_cancel':
			...
			{
				$bReleaseLock = nt3OwnershipLock::ReleaseLock($sObjClass, $iObjKey, $sToken);
			}
			break;
		...
		case 'new_dashlet':
			...
			if (is_subclass_of($sDashletClass, 'Dashlet'))
			{
				...
				$sHtml = addslashes($oForm->RenderAsPropertySheet($oPage, true /* bReturnHtml */, '.nt3-dashboard'));
				$sHtml = str_replace("\n", '', $sHtml);
				$sHtml = str_replace("\r", '', $sHtml);
...
EOF
			...
			$ant3Settings = array('cron_max_execution_time', 'timezone');
			...
			$oPage->add('<table>');
			$oPage->add('<tr>');
			$oPage->add('<td><a href='.NT3_HOME.' title='.NT3_HOME.' target="_blank" style="background: none;"><img src="../images/movicel.jpg?t='.utils::GetCacheBusterTimestamp().'" style="float: right;"/></a></td>');
			...
			$oPage->add("===== begin =====\n");
			$oPage->add('nt3Version: '.nt3_VERSION."\n");
			$oPage->add('nt3Build: '.nt3_REVISION."\n");
			$oPage->add('nt3BuildDate: '.nt3_BUILD_DATE."\n");
			...
			foreach($ant3Settings as $snt3Var)
			{
				$oPage->add('nt3Setting/'.$snt3Var.': '.MetaModel::GetConfig()->Get($snt3Var)."\n");
			}
...
EOF
		...
		case 'ticket_impact':
			...
			$sContextKey = 'nt3-tickets/relation_context/'.$sClass.'/'.$sRelation.'/'.$sDirection;
			$oAppContext = new ApplicationContext();
		...
		case 'extend_lock':
			...
			$iObjKey = (int)utils::ReadParam('obj_key', 0, false, 'integer');
			$sToken = utils::ReadParam('token', 0, false, 'raw_data');
			$aResult = nt3OwnershipLock::ExtendLock($sObjClass, $iObjKey, $sToken);
			if (!$aResult['status'])
			{
...
EOF
		...
		case 'custom_fields_update':
			...
			try
			{
				...
				$oRenderer = new \Combodo\nt3\Renderer\Console\ConsoleFormRenderer($oSubForm);
