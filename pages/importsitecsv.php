...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["site_file"]["size"] > 0) {
        ...
        $j = 2; $insFlag = TRUE; $parentSites = array();
        ...
        foreach ($csvData as $column) {
            ...
            if($header==1){
              $insFlag = FALSE;
                ...
                $query2 = "SELECT site_name FROM ntsites WHERE site_name = '".$column[0]."' AND is_active = 1";
            ...
            if($insFlag){
                ...
                if($result){
                    ...
                    $siteid = $conf->insert_id;
                    //$network = ($column[23]!='')? '2G':(($column[24]!='')? '3G':(($column[25]!='')? '4G':''));

                    if($column[23]!=''){
                        $queryNet1 = "INSERT INTO ntsitenetwork (`site_id`,`network`,`created_date`) VALUES (".$siteid.",'2G','".date('Y-m-d H:i:s')."')";
                        $resultNet1 = $conf->query($queryNet1);
                    }
                    if($column[24]!=''){
                        $queryNet2 = "INSERT INTO ntsitenetwork (`site_id`,`network`,`created_date`) VALUES (".$siteid.",'3G','".date('Y-m-d H:i:s')."')";
                        $resultNet2 = $conf->query($queryNet2);
                    }
                    if($column[25]!=''){
                        $queryNet3 = "INSERT INTO ntsitenetwork (`site_id`,`network`,`created_date`) VALUES (".$siteid.",'4G','".date('Y-m-d H:i:s')."')";
                        $resultNet3 = $conf->query($queryNet3);
                    }
                    ...
                    if($column[26]!=""){
                        $parentSiteId = 0;
                        $querySiteChk = "SELECT site_id FROM ntsites WHERE site_name='".$column[26]."' AND is_active = 1";
                        $resultSiteChk = $conf->query($querySiteChk);
                        if($resultSiteChk->num_rows>0){
                            $psitearr = mysqli_fetch_all($resultSiteChk,MYSQLI_ASSOC);
                            $queryUpdateParent = "UPDATE ntsites SET parent_site='".$psitearr[0]['site_id']."' WHERE site_id = $siteid";
                            $resultUpdateParent = $conf->query($queryUpdateParent);
                        }else{
                            $parentSites[$siteid] = $column[26];
                        }
                    }
          ...
          if(!empty($parentSites)){
            foreach ($parentSites as $siteid => $parent) {
                $querySiteChk = "SELECT site_id FROM ntsites WHERE site_name='".$parent."' AND is_active = 1";
                $resultSiteChk = $conf->query($querySiteChk);
                if($resultSiteChk->num_rows>0){
                    $psitearr = mysqli_fetch_all($resultSiteChk,MYSQLI_ASSOC);
                    $queryUpdateParent = "UPDATE ntsites SET parent_site='".$psitearr[0]['site_id']."' WHERE site_id = $siteid";
           ...
           if(!empty($result)) {
                $data = array('flag'=>TRUE,'msg'=>"All sites are added successfully" ,"parentSites"=>$parentSites);
                echo json_encode($data);
            } else {
                $data['msg'] = 'Unable to add site due to some problem or all sites are duplicated.';
                echo json_encode($data);
            }
