...
/**
 * This class manages the special template format used internally to build the NT3 web pages
 */
class DisplayTemplate
{
	...
	protected function RenderTag($oPage, $sTag, $aAttributes, $sContent)
	{
		...
		switch($sTag)
		{
			...
			case 'nt3tab':
				...
				$oTemplate = new DisplayTemplate($sContent);
				$oTemplate->Render($oPage, array()); // no params to apply, they have already been applied
				//$oPage->p('NT3 Tab Content:<pre>'.htmlentities($sContent, ENT_QUOTES, 'UTF-8').'</pre>');
				$oPage->SetCurrentTab('');
			break;
			...
			case 'nt3block': // No longer used, handled by DisplayBlock::FromTemplate see above
	...
	static public function UnitTest()
	{
		...
		<nt3block blockclass="HistoryBlock" type="toggle" encoding="text/oql">SELECT CMDBChangeOp WHERE objkey = $id$ AND objclass = \'$class$\'</nt3block>
		</div>
		<img src="../../images/connect_to_network.png" style="margin-top:-10px; margin-right:10px; float:right">
		<nt3tabs>
			<nt3tab name="Interfaces">
				<nt3block blockclass="DisplayBlock" type="list" encoding="text/oql">SELECT Interface AS i WHERE i.device_id = $id$</nt3block>
			</nt3tab>
			<nt3tab name="Contacts">
				<nt3block blockclass="DisplayBlock" type="list" encoding="text/oql">SELECT Contact AS c JOIN lnkContactToCI AS l ON l.contact_id = c.id WHERE l.ci_id = $id$</nt3block>
			</nt3tab>
			<nt3tab name="Documents">
				<nt3block blockclass="DisplayBlock" type="list" encoding="text/oql">SELECT Document AS d JOIN lnkDocumentToCI as l ON l.document_id = d.id WHERE l.ci_id = $id$)</nt3block>
			</nt3tab>
		</nt3tabs>';
		
		$oPage = new nt3WebPage('Unit Test');
