	...
	function GetBareProperties(WebPage $oPage, $bEditMode = false, $sPrefix, $aExtraParams = array())
	{
		...
		foreach($aDetailsStruct as $sTab => $aCols)
		{
			...
			foreach($aCols as $sColIndex => $aFieldsets)
			{
				...
				/*********************Edited by Nilesh for Network Field square box***********************/
				$sitevar++;
				if($sClass == 'Incident' || $sClass == 'Problem' || $sClass == 'EmergencyChange' || $sClass == 'NormalChange' || $sClass == 'RoutineChange'){
					...
					if($sitevar==2){
						...
						if($sEditMode=='view'){
							...
							$reasonData[0] = array('label'=>'<b>Sub Reason </b>','value'=>$subreasoninfo,'comments'=>'','infos'=>'','attcode'=>'sub_reason','layout'=>'small');
							$oPage->Details($reasonData);
						}else{
							...
							$reasonBlock = "<div id='field_2_origin' class='field_value_container'><div class='attribute-edit' data-attcode='origin'><div class='field_input_zone field_input_string' style='width:86%'><select name='reason' id='reason'><option value=''>-- Select One --</option>";
							...
							if(!empty($reasonArr)){				
								foreach ($reasonArr as $rData) {
									...
									$reasonBlock .= "<option value='".$rData['reason_id']."' $selected>".$rData['reason']."</option>";
								}
							}
							$reasonBlock .= "</select></div></div>";

							$reasonData[0] = array('label'=>'<b>Reason </b>','value'=>$reasonBlock,'comments'=>'','infos'=>'','attcode'=>'reason','layout'=>'small');
							$oPage->Details($reasonData);


							$subReasonBlock = "<div id='field_2_origin' class='field_value_container'><div class='attribute-edit' data-attcode='origin'><div class='field_input_zone field_input_string' style='width:86%'><select name='sub_reason' id='sub_reason'><option value=''>-- Select One --</option>";
							...
							if(isset($_GET['id']) && $reasonid!=0){
								...
								if(!empty($subreasonArr)){				
									foreach ($subreasonArr as $rData) {
										...
										$subReasonBlock .= "<option value='".$rData['sub_reason_id']."' $selected>".$rData['sub_reason']."</option>";
									}
								}
							}

							$subReasonBlock .= "</select><img src='../images/indicator.gif' style='float: right;padding-top: 10px;display:none' id='reasonLoad'></div></div>";
...
EOF
					...
					if($sitevar==3){
						if($sEditMode=='view'){
							...
							$existedProvince = "";$existedProvider = "";$existedNetwork = "";$existedCompType = "";
							$existedAftdService = array(); $existedEvent="";$existedCategory="";
							...
							if(isset($_GET['id'])){
								...
								$existProvinceData = CMDBSource::QueryToArray("SELECT tk.province_id,tk.provider_id,tk.aftd_network_id,tk.aftd_comp_type_id,tk.event_id,tk.category_id FROM ntticket tk WHERE tk.id = ".$ticketID);	
					...
					if($sitevar == $SiteBlock){
						if($sEditMode=='view'){
							...
							$oPage->add('<fieldset>');
							//$oPage->add('<legend>Components</legend>');

							/************ Events ***************/
							$eventData = CMDBSource::QueryToArray("SELECT * FROM ntevent WHERE is_active = 1");
							$eventsdd = "<div id='field_2_origin' class='field_value_container'><div class='attribute-edit' data-attcode='origin'><div class='field_input_zone field_input_string' style='width:86%'><select name='event'><option value=''>-- Select One --</option>";
							if(!empty($eventData)){
								foreach ($eventData as $eData) {
									
									$selected = ($existedEvent==$eData['event_id'])? "selected='selected'":"";
									$eventsdd .= "<option value='".$eData['event_id']."' ".$selected.">".$eData['event']."</option>";
								}
							}
							$eventsdd .= "</select></div></div></div>";
							...
							$categorydd = "<div id='field_2_origin' class='field_value_container'><div class='attribute-edit' data-attcode='origin'><div class='field_input_zone field_input_string' style='width:86%'><select name='category'><option value=''>-- Select One --</option>";
							if(!empty($categoryData)){
								foreach ($categoryData as $cData) {
									$selected = ($existedCategory==$cData['category_id'])? "selected='selected'":"";
									$categorydd .= "<option value='".$cData['category_id']."' ".$selected.">".$cData['category']."</option></div></div></div>";
								}
							}
							$categorydd .= "</select>";
						...
						} // EOF else edit or new 4th

					}// EOF 4th square box

					$oPage->add('</fieldset>');

				} // EOF incident,problem or change condition
