...
define('nt3_CONFIG_FILE', 'config-nt3.php');
define('nt3_DEFAULT_CONFIG_FILE', APPCONF.nt3_DEFAULT_ENV.'/'.nt3_CONFIG_FILE);
...
class utils
{
	...
	public static function InitTimeZone() {
		...
		$snt3TimeZone = $oConfig->Get('timezone');

		if (!empty($snt3TimeZone))
		{
			date_default_timezone_set($snt3TimeZone);
		}
		else
		{
	 ...
	 * 1) NT3 is using an internal authentication
	...
	/**
	 * Execute the given NT3 PHP script, passing it the current credentials
	 ...
	 * @param string $sScript Name and relative path to the file (relative to the NT3 root dir)
	 * @param hash $aArguments Associative array of 'arg' => 'value'
	 * @return array(iCode, array(output lines))
	 */	 	  
	/**
	 */
	static function Execnt3Script($sScriptName, $aArguments)
	{
	...
	{
		if (isset($_SESSION['nt3_env']))
		{
			return $_SESSION['nt3_env'];
		}
		else
		{
			return nt3_DEFAULT_ENV;
		}
	}
	...
	public static function GetConfigFilePath($sEnvironment = null)
	{
		...
		return APPCONF.$sEnvironment.'/'.nt3_CONFIG_FILE;
	}
	...
	/**
	 * Check if the given URL is a link to download a document/image on the CURRENT NT3
	...
	public static function IsSelfURL($sPath)
	{
		...
		{
			// If the URL is an URL pointing to this instance of NT3, then
			...
			throw new Exception('Invalid URL. This NT3 URL is not pointing to a valid Document/Image.');
		}
		return $result;
	}
	 ...
	 * - an URL pointing to a blob (image/document) on the current NT3 server
	 * - an http(s) URL
	 ...
	 * the version of NT3. This string is useful to prevent browser side caching of content that may vary at each
	 * (re)installation of NT3 (especially during development). 
	 * @return string
	 */
