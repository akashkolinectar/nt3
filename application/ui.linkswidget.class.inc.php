...
EOF
	...
	protected function DisplayFormTable(WebPage $oP, $aConfig, $aData)
	{
		...
		foreach($aConfig as $sName=>$aDef)
		{
			...
			/************Edited by PRIYA**************/
			$label = $aDef['label'];
			if(isset($this->m_sAttCode) && $this->m_sAttCode=='functionalcis_list' && $sName=='static::key'){
				$label = "Function Element";
			}
			/**************END************/
			$sHtml .= "<th title=\"".$aDef['description']."\">".$label."</th>\n";
		}
		$sHtml .= "</tr>\n";
		$sHtml .= "</thead>\n";
...
EOF
		...
		$buttonLabel = MetaModel::GetName($this->m_sRemoteClass);
		if(isset($this->m_sAttCode) && $this->m_sAttCode=='functionalcis_list'){
			$buttonLabel = "Function Element";
		}
		if(isset($this->m_sAttCode) && $this->m_sAttCode=='cis_list'){
			$buttonLabel = "Function Element";
		}
		if(isset($this->m_sAttCode) && $this->m_sAttCode=='ci_list'){
			$buttonLabel = "Function Element";
		}
		/***************End*********/
		/********** Edited By Nilesh New For adding document link in contract page *****************/	
		if($this->m_sAttCode=='documents_list' || $this->m_sAttCode=='document_list'){
			$sHtmlValue .= "&nbsp;&nbsp;&nbsp;<input id=\"{$this->m_sAttCode}{$this->m_sNameSuffix}_btnCreateNewDoc\" type=\"button\" value=\"Create New Document\" onClick=\"window.location.href='https://nt3.nectarinfotel.com/pages/UI.php?operation=new&amp;class=Document&amp;c[menu]=Document'\" >";
		}
		/********** EOF Edited By Nilesh New For adding document link in contract page ************/

		$sHtmlValue .= "<span style=\"float:left;\">&nbsp;&nbsp;&nbsp;<img src=\"../images/tv-item-last.gif\">&nbsp;&nbsp;<input id=\"{$this->m_sAttCode}{$this->m_sNameSuffix}_btnRemove\" type=\"button\" value=\"".Dict::S('UI:RemoveLinkedObjectsOf_Class')."\" onClick=\"oWidget{$this->m_iInputId}.RemoveSelected();\" >";
		$sHtmlValue .= "&nbsp;&nbsp;&nbsp;<input id=\"{$this->m_sAttCode}{$this->m_sNameSuffix}_btnAdd\" type=\"button\" value=\"".Dict::Format('UI:AddLinkedObjectsOf_Class', $buttonLabel)."\" onClick=\"oWidget{$this->m_iInputId}.AddObjects();\"><span id=\"{$this->m_sAttCode}{$this->m_sNameSuffix}_indicatorAdd\"></span></span>\n";
	...
	public function GetObjectPickerDialog($oPage, $oCurrentObj, $sJson, $aAlreadyLinkedIds = array(), $aPrefillFormParam = array())
	{
		...
		$oPage->add_ready_script("$('#dlg_{$this->m_sAttCode}{$this->m_sNameSuffix}').dialog('option', "
                . "{title:'"."Add Link Functional Element / Ticket objects to link with the Incident"."'});");
