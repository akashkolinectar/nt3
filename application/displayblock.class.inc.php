...
class DisplayBlock
{
	...
	public static function FromTemplate($sTemplate)
	{
		...
		$snt3Data = substr($sTemplate, 1+$iEndTag, $iEndPos - $iEndTag - 1);
		$snt3Tag = substr($sTemplate, $iStartPos + strlen('<'.self::TAG_BLOCK), $iEndTag - $iStartPos - strlen('<'.self::TAG_BLOCK));
		...
		if (preg_match('/ type="(.*)"/U',$snt3Tag, $aMatches))
		{
			$sBlockType = strtolower($aMatches[1]);
		}
		if (preg_match('/ asynchronous="(.*)"/U',$snt3Tag, $aMatches))
		{
			$bAsynchronous = (strtolower($aMatches[1]) == 'true');
		}
		if (preg_match('/ blockclass="(.*)"/U',$snt3Tag, $aMatches))
		{
			$sBlockClass = $aMatches[1];
		}
		if (preg_match('/ encoding="(.*)"/U',$snt3Tag, $aMatches))
		{
			$sEncoding = strtolower($aMatches[1]);
		}
		if (preg_match('/ link_attr="(.*)"/U',$snt3Tag, $aMatches))
		{
		...
		if (preg_match('/ target_attr="(.*)"/U',$snt3Tag, $aMatches))
		{
			// The list to display is a list of links to the specified object
			$aParams['target_attr'] = $aMatches[1]; // Name of the Ext. Key that make this linkage
		}
		if (preg_match('/ object_id="(.*)"/U',$snt3Tag, $aMatches))
		{
			// The list to display is a list of links to the specified object
		...
		if (preg_match('/ parameters="(.*)"/U',$snt3Tag, $aMatches))
		{
		...
		{
			case 'text/serialize':
			$oFilter = DBSearch::unserialize($snt3Data);
			break;
			
			case 'text/oql':
			$oFilter = DBSearch::FromOQL($snt3Data);
			break;
		}
		...
		if (!$this->m_bAsynchronous)
		{
			...
			if(isset($_GET['c'])){
				if(isset($_GET['c']['menu'])){
					switch ($_GET['c']['menu']) {
						case 'Incident:Overview': $bDisplayClass = ' IncidentOverview_'.$sId; break;
						case 'Problem:Overview': $bDisplayClass = ' ProblemOverview_'.$sId; break;
						case 'Change:Overview': $bDisplayClass = ' ChangeOverview_'.$sId; break;
						default: break;
					}
				}
			}

			$sHtml .= "<div id=\"$sId\" class=\"display_block$bDisplayClass\" >\n";
			try
			{
...
//            $oPage->add_ready_script("console.debug($('#Menu_UserRequest_OpenRequests').data());");
	...
	public function GetRenderContent(WebPage $oPage, $aExtraParams = array(), $sId)
	{
		...
		switch($this->m_sStyle)
		{
			...
			else
			{
				...
				else
				{
					...
					/************ Modified by Nilesh New For add create document link on popup ****************/
					if(($aExtraParams['table_id']=='add_documents_list' || $aExtraParams['table_id']=='add_document_list') && $aExtraParams['selection_mode']==1){ 
						$bDisplayMenu = TRUE;
					}
					/************ EOF Modified by Nilesh New For add create document link on popup ****************/ 
					if ($bDisplayMenu)
					{
				...
				if (isset($aExtraParams['update_history']) && true == $aExtraParams['update_history'])
				{
					...
					$seventAttachedData = json_encode(array(
						...
						'breadcrumb_instance_id'=> MetaModel::GetConfig()->Getnt3Instanceid(),
						'breadcrumb_icon'       => utils::GetAbsoluteUrlAppRoot().'images/breadcrumb-search.png'
					));
					$oPage->add_ready_script("$('body').trigger('update_history.nt3', [$seventAttachedData])");
				}
			}
			break;
			...
			//for displaying contact only done by@rekha
			
			// echo $sClass."\n";
			
			switch ($sClass) {
				case 'Location': case 'Contact': case 'BusinessProcess': case 'ApplicationSolution': case 'DBServer': case 'DatabaseSchema': case 'MobilePhone': case 'Middleware': case 'MiddlewareInstance': case 'NAS': case 'NetworkDevice': case 'PC': case 'PCSoftware': case 'PDU': case 'Rack': case 'VirtualMachine':
					$sHtml .= MetaModel::GetClassIcon($sClass, true, 'float;left;margin-right:10px;border:0;');

						//$sHtml .= MetaModel::GetName($sClass).': '.$iCount.'</a></p>';
						$sHtml .= MetaModel::GetName($sClass).'</a></p>';
						//echo($sClass);
						$sParams = $oAppContext->GetForLink();
						$sHtml .= '<p>';
		...
		/*	if($sClass=='Contact' ){
			...
			case 'summary':
			//commenting helpdesk on dashboiard @Rekha
			...
			/*********** Edited By Nilesh For Overview Graph Header Label ***************/
			...
			$sTitle = isset($aExtraParams['chart_title']) ? htmlentities(Dict::S($aExtraParams['chart_title']), ENT_QUOTES, 'UTF-8') : ''; // Here removed h1 tag
			
			if(isset($_COOKIE['pre_days']) && isset($_COOKIE['new_days'])){			
				$dd = (int)$_COOKIE["new_days"];				
				/*$sTitle = str_replace('20', $dd, $sTitle);
				$sTitle = str_replace('14', $dd, $sTitle);
				if(strlen($dd)==1){
					$sTitle = str_replace('7', $dd, $sTitle);
				}*/
				preg_match('!\d+!', $sTitle, $matches);
				if(isset($matches[0])){
					$sTitle = str_replace($matches[0], $dd, $sTitle);
				}
				if($sClass=='Problem'){
					$sTitle = $sTitle." ($dd days)";
				}
				/*$sTitle = str_replace('últimos 7 dias', $dd, $sTitle);*/
			}

			$sTitle = '<h1 style="text-align:center">'.$sTitle.'</h1>'; // Added h1 tag here

			/*********** EOF Edited By Nilesh For Overview Graph Header Label ***************/
			
			$sHtml = "$sTitle<div style=\"height:260px;width:100%\" id=\"my_chart_$sId{$iChartCounter}\"><div style=\"height:200px;line-height:200px;vertical-align:center;text-align:center;width:100%\"><img src=\"../images/indicator.gif\"></div></div>\n";

			$oPage->add_ready_script(
<<<EOF

			/******************** Edited By Nilesh New For PDF Overview *********************/
				...
				generate = function(){
					...
					var ul = $("<ul>");
					$('.display_block').each(function(){
				        var li = $("<li style='margin-top:25px;margin-bottom:20px;height:auto;'>");
				        var head = $(this).siblings("h1").html();
				        if(head!=undefined){
				        	li.append("<h1 style='text-align:center;margin-top:20px'>"+head+"</h1>");	
				        }
				        li.append($(this).html());
				        ul.append(li);
			        });
			        ...
			        $('#previewMain').append("<style type='text/css'>ul{list-style-type: none;}</style>");
					var element = document.getElementById('previewMain');
					var opt = {
					  margin:       1,
					  filename:     'NT3Overview.pdf',
			...
			/******************** EOF Edited By Nilesh New For PDF Overview *********************/
EOF
			);
...
EOF
			...
			switch($sChartType)
			{
				...
				$oPage->add_ready_script(
<<<EOF
	var newDay1 = $("#days").val();

	if($sJson.length === 0)
	{

		$('#my_chart_block_22').html('<span class="ticketmessage">Ticket Not Created within ' + newDay1 + ' days</span>');
		$('#my_chart_block_12').html('<span class="ticketmessage">Ticket Not Created within ' + newDay1 + ' days</span>');
		$('.c3').html('<span class="ticketmessage">Ticket Not Created within ' + newDay1 + ' days</span>');
...
EOF
				...
				switch ($_SESSION['language']) {
					case 'PT BR':
						foreach (json_decode($sJSNames) as $key => $value) {
							if(strcasecmp($value,'Baixa') == 0){$colordata .= $key.':"rgb(44, 160, 44)",';}
							if(strcasecmp($value,'Média') == 0){$colordata .= $key.':"#FCFE7C",';}
							if(strcasecmp($value,'Alta') == 0){$colordata .= $key.':"rgb(255, 127, 14)",';}
							if(strcasecmp($value,'Crítica') == 0){$colordata .= $key.':"rgb(214, 39, 40)",';}
						}
						break;
					
					default: 
						foreach (json_decode($sJSNames) as $key => $value) {
							if(strcasecmp($value,'low') == 0){$colordata .= $key.':"rgb(44, 160, 44)",';}
							if(strcasecmp($value,'medium') == 0){$colordata .= $key.':"#FCFE7C",';}
							if(strcasecmp($value,'high') == 0){$colordata .= $key.':"rgb(255, 127, 14)",';}
							if(strcasecmp($value,'critical') == 0){$colordata .= $key.':"rgb(214, 39, 40)",';}
						}
						break;
				}
				$colordata .= "}";
...
<<<EOF
	...
	if($sJSColumns.length === 0 || $sJSColumns.length =='')
	{
		$('#my_chart_block_01').html('<span class="ticketmessage">Ticket Not Created within ' + newDay1 + ' days</span>');
		$('#my_chart_block_11').html('<span class="ticketmessage">Ticket Not Created within ' + newDay1 + ' days</span>');
...
EOF
	...
	protected function MakeGroupByQuery(&$aExtraParams, &$oGroupByExp, &$sGroupByLabel, &$aGroupBy, &$sAggregationFunction, &$sFctVar, &$sAggregationAttr, &$sSql)
	{
		...
		if (isset($aExtraParams['aggregation_function']) && !empty($aExtraParams['aggregation_attribute']))
		{
			...
			$sFctVar = '_nt3_'.$sAggregationFunction.'_';
	...
	public function GetRenderContent(WebPage $oPage, $aExtraParams = array(), $sId)
	{
		...
		switch($oSet->Count())
		{
			...
			else
			{
				...
				{
					$aLockInfo = nt3OwnershipLock::IsLocked(get_class($oObj), $id);
					if ($aLockInfo['locked'])
					{
						$bLocked = true;
		...
		if (!$oPage->IsPrintableVersion())
		{
			if($sClass=='ProviderContract'){
				$sHtml .= "<button type='button' class='action' style='margin: 0px 3px;cursor:pointer;' id='exportSLA'>Export SLA</button>";
				$sHtml .= "<div id='slaDates' style='display:none;'><form id='getDateSla' name='getDateSla'> From Date: <input type='date' name='fromDate' id='fromDate' placeholder='Select From Date' required max='".date('Y-m-d')."'> <br/> Till Date: <input type='date' name='toDate' id='toDate' placeholder='Select Till Date' style='margin-left: 17px;' required max='".date('Y-m-d')."'> <br/><button type='submit' value='Get Report' class='action' style='float:right;cursor:pointer;margin-top: 15px;'>Get Report</button></form></div>";
				$oPage->add_ready_script(
<<<EOF
					$(document).on('click','#exportSLA',function(){
						$("#slaDates").dialog();
					});
					...
					$(document).on('submit','#getDateSla',function(event){
						event.preventDefault();
						var fromDate = $("#fromDate").val();
						var toDate = $("#toDate").val();
						window.location.href="https://nt3.nectarinfotel.com/webservices/getSLAReport.php?fromDate="+fromDate+"&toDate="+toDate;
					});
EOF
				);
			}
			...
			{
				$sHtml .= "<div class=\"nt3_popup actions_menu\"><ul>\n<li>".Dict::S('UI:Menu:OtherActions')."\n<ul>\n";
			}
			else
			{
		...
		if (!$bPopupScript)
		{
			...
			$oPage->add_ready_script("$(\"div.nt3_popup>ul\").popupmenu();\n");
			$bPopupScript = true;
		}
		return $sHtml;
	}
