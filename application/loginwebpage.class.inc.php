...
class LoginWebPage extends NiceWebPage
{
	...
	public function DisplayLoginHeader($bMainAppLogo = false)
	{
		...
		{
			$sLogo = 'nt3-logo-external.png';
			$sBrandingLogo = 'login-logo.png';
		}

		/*
		$sVersionShort = Dict::Format('UI:nt3Version:Short', nt3_APPLICATION, nt3_VERSION);
		*/

		$sVersionShort = Dict::Format(nt3_APPLICATION, nt3_VERSION);
	...
	public function DisplayLoginForm($sLoginType, $bFailedLogin = false)
	{
		switch($sLoginType)
		{
			...
			case 'basic':
			case 'url':
			$this->add_header('WWW-Authenticate: Basic realm="'.Dict::Format('UI:nt3Version:Short', nt3_APPLICATION, nt3_VERSION));
			...
			$this->add("<tr><td style=\"text-align:left\"><input id=\"user\" type=\"text\" name=\"auth_user\" value=\"".htmlentities($sAuthUser, ENT_QUOTES, 'UTF-8')."\"  placeholder=\"".Dict::S('UI:Login:UserNamePrompt')."\"  /></td></tr>\n");
			
			$this->add("<tr><td style=\"text-align:left\"><input id=\"pwd\" type=\"password\" name=\"auth_pwd\" value=\"".htmlentities($sAuthPwd, ENT_QUOTES, 'UTF-8')."\" placeholder=\"".Dict::S('UI:Login:PasswordPrompt')."\" /></td></tr>\n");
	...
	public function ForgotPwdLink()
	{
		...
		$movicel_img = utils::GetAbsoluteUrlAppRoot().'images/movicel.jpg';
		$sHtml .="</br><img alt='Movicel' src=\"$movicel_img\">";
		//$sHtml ="</br><img alt='Movicel' src=\"$movicel_img\">";
	...
	public function DisplayForgotPwdForm($bFailedToReset = false, $sFailureReason = null)
	{
		...
		$this->add("<tr><td colspan=\"2\" class=\"center\"><label for=\"user\">Email ID:</label><input id=\"user\" type=\"text\" name=\"auth_user\" value=\"".htmlentities($sAuthUser, ENT_QUOTES, 'UTF-8')."\" /></td></tr>\n");

		$this->add("<tr><td colspan=\"2\" class=\"center v-spacer\"> <span class=\"btn_border\"><input type=\"submit\" value=\"".Dict::S('UI:Login:ResetPassword')."\" /></span> <span class=\"btn_border\"><input type=\"button\" onClick=\"window.close();\" style=\"border-radius: 24px;\" value=\"".Dict::S('UI:Button:Cancel')."\" /></span></td></tr>\n");
...
EOF
	...
	protected static function Login($iOnExit)
	{
		...
		while(($sLoginMode == '') && ($index < count($aAllowedLoginTypes)))
		{
			...
			switch($sLoginType)
			{
				...
				case 'form':
				// NT3 standard mode: form based authentication
		...
		if ($sLoginMode == '')
		{
			...
			{
				header('WWW-Authenticate: Basic realm="'.Dict::Format('UI:nt3Version:Short', nt3_APPLICATION, nt3_VERSION));
				header('HTTP/1.0 401 Unauthorized');
	...
	protected static function HandleOperations($operation)
	{
		...
		{
			/****** Edited By Nilesh For First Login Change Password *********/
			if(isset($_GET['auth_user'])){
				$_SESSION['auth_user'] = $_GET['auth_user'];
			}
			/****** EOF Edited By Nilesh For First Login Change Password *********/
		...
		if ($operation == 'do_change_pwd')
		{
			...
			/****** Edited By Nilesh For First Login Change Password *********/
			$sUpdateQuery = "UPDATE `ntpriv_user` SET `first_user`= 0 WHERE `login` = '".$sAuthUser."'";
