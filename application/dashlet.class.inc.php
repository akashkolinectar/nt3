 ...
 *
 * Used as a fallback in NT3 for unknown dashlet classes.
 *
 * @since 2.5
 */
...
class DashletObjectList extends Dashlet
{
	...
	public function Render($oPage, $bEditMode = false, $aExtraParams = array())
	{
		// To display service overview Edited by Nilesh
        ...
        if($sShowMenu==0){
			$oPage->add('<div class="dashlet-content">');
			//to remove user request done by @rekha
			$sHtmlTitle = htmlentities(Dict::S($sTitle), ENT_QUOTES, 'UTF-8'); // done in the NT3 block
			//echo $sHtmlTitle;

			/*if($sHtmlTitle=="Customer contracts to be renewed in 30 days" || $sHtmlTitle=="Provider contracts to be renewed in 30 days"){*/

		/************ Modified by Nilesh new to show service overview for other language*****************/

			if($sTitle=="UI-ServiceManagementOverview-CustomerContractToRenew" || $sTitle=="UI-ServiceManagementOverview-ProviderContractToRenew"){

		/************ EOF Modified by Nilesh new to show service overview for other language*****************/
	...
	public function RenderNoData($oPage, $bEditMode = false, $aExtraParams = array())
	{
		...
		$oPage->add('<div class="dashlet-content">');
		$sHtmlTitle = htmlentities($this->oModelReflection->DictString($sTitle), ENT_QUOTES, 'UTF-8'); // done in the NT3 block
		if ($sHtmlTitle != '')
		{
			$oPage->add('<h1>'.$sHtmlTitle.'</h1>');
		}
...
abstract class DashletGroupBy extends Dashlet
{
		...
		else
		{
			switch($sStyle)
			{
				case 'bars':
					...
					$sHtmlTitle = ''; // done in the NT3 block
					break;
			...
			if ($sHtmlTitle != '')
			{
				$sHtmlTitle = str_replace('customer', 'department', $sHtmlTitle);
				/*********** Edited By Nilesh For Overview Graph Header Label ***************/
				if(isset($_COOKIE['pre_days']) && isset($_COOKIE['new_days'])){			
					$dd = (int)$_COOKIE["new_days"];				
					/*$sHtmlTitle = str_replace('20', $dd, $sHtmlTitle);
					$sHtmlTitle = str_replace('14', $dd, $sHtmlTitle);
					if(strlen($dd)==1){
						$sHtmlTitle = str_replace('7', $dd, $sHtmlTitle);
					}*/
					preg_match('!\d+!', $sHtmlTitle, $matches);
					if(isset($matches[0])){
						$sHtmlTitle = str_replace($matches[0], $dd, $sHtmlTitle);
					}
					if($sClass=='Problem'){
						$sHtmlTitle = $sHtmlTitle." ($dd days)";
					}
				}
	...
	protected function GetOrderBy()
	{
		...
		return array(
			...
			'_nt3_'.$this->aProperties['aggregation_function'].'_' => Dict::S('UI:GroupBy:'.$this->aProperties['aggregation_function']));
	}
