...
class UIExtKeyWidget
{
	...
	public function Display(WebPage $oPage, $iMaxComboLength, $bAllowTargetCreation, $sTitle, DBObjectset $oAllowedValues, $value, $iInputId, $bMandatory, $sFieldName, $sFormPrefix = '', $aArgs = array(), $bSearchMode = null, $sDisplayStyle = 'select', $bSearchMultiple = true)
	{
		...
		/***************** Edited By Nilesh for enable parent problem/change management search ********************/
		
		if($sFieldName=='parent_problem_id' || $sFieldName=='parent_change_id' || $sFieldName == 'parent_id'
		   || $sFieldName=='problem_id' || $sFieldName=='related_change_id' || $sFieldName=='caller_id'){
			$searchFlag = true;
		}else{
			$searchFlag = $oAllowedValues->CountExceeds($iMaxComboLength);
		}
		...
		{*/
			...
			switch($sDisplayStyle)
			{
				...
				STATIC $funCIOrg = ""; // Edited by Nilesh for add/modify new sub-functional CI
				...
				if ($this->bSearchMode)
				{
					...
					else
					{
						...
						$sDisplayValue = isset($aArgs['sDefaultValue']) ? 'All Department' : Dict::S('UI:SearchValue:Any');
				...
				{
					/************ Edited by Nilesh for add/modify new sub-functional CI ************/
					if($sFieldName == "org_id" && $funCIOrg==""){
						$funCIOrg = "<select name=\"functionalci_org\" id=\"functionalci_org\">\n<option value=\"\">".Dict::S('UI:SelectOne')."</option>\n";
					}
					/************ EOF Edited by Nilesh for add/modify new sub-functional CI ************/
				...
				while($oObj = $oAllowedValues->Fetch())
				{
					...
					/************ Edited by Nilesh for add/modify new sub-functional CI ************/
					if($sFieldName == "org_id"){
						$funCIOrg .= "<option value=\"$key\" $sSelected>$display_value</option>\n";
					}
					/************ EOF Edited by Nilesh for add/modify new sub-functional CI ************/
				...
				/******************** Modified by Nilesh for Edit option in more information CI **************************/

					switch ($sFieldName) {
						case 'brand_id': $finalClass='Brand';break;						
						case 'model_id': $finalClass='Model';break;						
						case 'osfamily_id': $finalClass='OSFamily';break;						
						case 'osversion_id': $finalClass='OSVersion';break;						
						case 'networkdevicetype_id': $finalClass='NetworkDeviceType';break;						
						case 'iosversion_id': $finalClass='IOSVersion';break;
						case 'system_id': $finalClass='FunctionalCI'; $headerName='System';
						$subClassName = 'Server'; $subClassArr = "('Server','VirtualMachine')"; break;
						default: $finalClass='';break;
					}
...
if($finalClass=='FunctionalCI'){
	try{
			
			$sHTMLValue .= '<span class="field_input_btn editFunctionalCI'.$sFieldName.'">
			<img id="editFunctionalCImg_'.$sFieldName.'" style="border:0;vertical-align:middle;cursor:pointer;" src="../images/wrench.png"></span><span class="field_input_btn newFunctionalCI'.$sFieldName.'"><img id="newFunctionalCImg_'.$sFieldName.'" style="border:0;vertical-align:middle;cursor:pointer;" src="../images/mini_add.gif"></span>';

			$subCIModule = CMDBSource::QueryToArray("SELECT * FROM ntfunctionalci WHERE finalclass IN ".$subClassArr." ORDER BY name DESC");



					$selectBox = "<select id='functionalCI".$sFieldName."' name='functional_ci'><option value=''> -- Select One -- </option>";
					foreach ($subCIModule as $rows) {
						$selectBox .= "<option value='".$rows['id']."'>".$rows['name']."</option>";
					}
					$selectBox .= "</select>";

					$sHTMLValue .= '<div id="addFunctionalCIDialog'.$sFieldName.'" class="modal">
									  	<h1>Add '.$headerName.' </h1>
									  	<label> Department : </label><br/>
									  	'.$funCIOrg.'<span class="form_validation">
							    			<img src="../images/validation_error.png" style="vertical-align:middle" title="Please specify a value">
							    		</span>
							    		<br/><br/>
									    <label> '.$headerName.' Name : </label><br/>
									    <input type="text" name="functionalci_name" id="functionalci_name'.$headerName.'">
									    <span class="form_validation">
							    			<img src="../images/validation_error.png" style="vertical-align:middle" title="Please specify a value">
							    		</span>
									    <input type="button" class="addFunctionalCI'.$headerName.'" value="Create" style="padding: 5px 17px 4px 17px;border-radius: 2px;color: #ffffff;background-color: #F17422;border: 1px solid #F17422;cursor: pointer;float: right;">
									    <div class="msgLoad" style="padding-top: 10px;float:right;"></div>
								    </div>';

					    $sHTMLValue .= '<div id="editFunctionalCIDialog'.$sFieldName.'" class="modal">
									  	<h1>Edit '.$headerName.' </h1>
									  	<label> '.$headerName.' : </label><br/>
									  	'.$selectBox.'<span class="form_validation">
							    			<img src="../images/validation_error.png" style="vertical-align:middle" title="Please specify a value">
							    		</span>
							    		<br/><br/>
									    <label> New '.$headerName.' : </label><br/>
									    <input type="text" name="functionalci_new_name" id="functionalci_new_name'.$headerName.'">									    
									    <span class="form_validation">
							    			<img src="../images/validation_error.png" style="vertical-align:middle" title="Please specify a value">
							    		</span>
									    <input type="button" class="editFunctionalCI'.$headerName.'" value="Update" style="padding: 5px 17px 4px 17px;border-radius: 2px;color: #ffffff;background-color: #F17422;border: 1px solid #F17422;cursor: pointer;float: right;">
									    <div class="msgLoad" style="padding-top: 10px;float:right;"></div>
								    </div>';
...
<<<EOF
	...
	$(".newFunctionalCI$sFieldName").on("click",function(){
		$( "#addFunctionalCIDialog$sFieldName" ).dialog();
	});

	$(".editFunctionalCI$sFieldName").on("click",function(){
		$( "#editFunctionalCIDialog$sFieldName" ).dialog();
	});
	$("#functionalCI$sFieldName").on("change",function(){
		$("#functionalci_new_name$headerName").val($("#functionalCI$sFieldName option:selected").text());
	});	
	
	$(".addFunctionalCI$headerName").on("click",function(){		
		
		var org = $("#functionalci_org").val();
		var fciname = $("#functionalci_name$headerName").val();
		
		if(org=="" || fciname==""){
			alert("Please fill all mandatory fields");
		}else{
			$(this).attr("disabled","disabled");
			$(".msgLoad").html("<img src='../images/indicator.gif'>");
			$.ajax({
				url: "addSiteAttr.php",
				data: {
					"attr":'FunctionalCIAdd',
					"organization":org,
					"ciname":fciname,
					"finalclass":'$subClassName'
					},
				type: "POST",
				dataType: "json",
				success: function(res){
					console.log(res);
					if(res){
						$(".addFunctionalCI$headerName").removeAttr("disabled");
						$(".msgLoad").html("");
						alert("$headerName added successfully");
						location.reload();
					}
				}
			});
	...
	$(".editFunctionalCI$headerName").on("click",function(){
		var cid = $("#functionalCI$sFieldName").val();
		var newname = $("#functionalci_new_name$headerName").val();
		if(cid=="" || newname==""){
			alert("Please fill all mandatory fields");
		}else{
			$(this).attr("disabled","disabled");
			...
			$.ajax({
				...
				success: function(res){
					console.log(res);
					if(res){
						$(".editFunctionalCI$headerName").removeAttr("disabled");
						$(".msgLoad").html("");
						alert("$headerName updated successfully");
						location.reload();
					}
				}
			});
...
} // EOF If Functional CI
else{
		
	$sHTMLValue .= '<span class="field_input_btn editMoreInfo'.$sFieldName.'"><img id="mini_edit_'.$sFieldName.'" style="border:0;vertical-align:middle;cursor:pointer;" src="../images/wrench.png?t=1538568981.6184"></span>';
					
					$subStageModule = CMDBSource::QueryToArray("SELECT * FROM nttypology WHERE finalclass = '".$finalClass."' ORDER BY name DESC");
					$selectBox = "<select id='typology".$sFieldName."' name='typology'><option value=''> -- Select One -- </option>";
					foreach ($subStageModule as $rows) {
						$selectBox .= "<option value='".$rows['id']."'>".$rows['name']."</option>";
					}
					$selectBox .= "</select>";

					$sHTMLValue .= '<div id="editMoreInfoDialog'.$sFieldName.'" class="modal">
									  	<h1>Modify '.$finalClass.' </h1>
									  	<p></p>
									    <label>Select '.$finalClass.' : </label><br/>'.$selectBox.'<span class="form_validation">
							    			<img src="../images/validation_error.png" style="vertical-align:middle" title="Please specify a value">
							    		</span>
							    		<br/><br/>
									    <label> Updated '.$finalClass.' Name : </label><br/>
									    <input type="text" name="typology_name" id="typology_name'.$sFieldName.'">
									    <span class="form_validation">
							    			<img src="../images/validation_error.png" style="vertical-align:middle" title="Please specify a value">
							    		</span>
									    <input type="button" class="modifyTypology'.$finalClass.'" value="Update" style="padding: 5px 17px 4px 17px;border-radius: 2px;color: #ffffff;background-color: #F17422;border: 1px solid #F17422;cursor: pointer;float: right;">
									    <div class="msgLoad" style="padding-top: 10px;float:right;"></div>
								    </div>';
...
<<<EOF
	...
	$("#typology$sFieldName").on("change",function(){
		$("#typology_name$sFieldName").val($("#typology$sFieldName option:selected").text());
	});
	$(".editMoreInfo$sFieldName").on("click",function(){
		$( "#editMoreInfoDialog$sFieldName" ).dialog();
	});
	$(".modifyTypology$finalClass").on("click",function(){		
		var attr = "typology";
		var typologyName = $("#typology_name$sFieldName").val();
		var typology =  $("#typology$sFieldName").val();
		if(typology=="" || typologyName==""){
			alert("Please fill all mandatory fields")
		}else{
			$(this).attr("disabled","disabled");
			$(".msgLoad").html("<img src='../images/indicator.gif'>");
			$.ajax({
				...
				success: function(res){
					console.log(res);
					if(res){
						$(".modifyTypology$finalClass").removeAttr("disabled");
						$(".msgLoad").html("");
						alert("$finalClass modified successfully");
						location.reload();
					}
				}
			});
...
EOF
	...
	} // EOF Else Not FinctionalCI


}	// EOF Finalclass Check

	/******************** EOF Modified by Nilesh for Edit option in more information CI **************************/
...
<<<EOF
		...
		/*************** Edited By Nilesh New for regardless department show callers *******************/
		//var isCaller = '$this->sAttCode';
		//if( isCaller != 'caller_id'){
			oACWidget_{$this->iId}.emptyHtml = "<div style=\"background: #fff; border:0; text-align:center; vertical-align:middle;\"><p>$sMessage</p></div>";
		$('#label_$this->iId').autocomplete(GetAbsoluteUrlAppRoot()+'pages/ajax.render.php', { scroll:true, minChars:{$iMinChars}, autoFill:false, matchContains:true, mustMatch: false, keyHolder:'#{$this->iId}', extraParams:{operation:'ac_extkey', sTargetClass:'{$this->sTargetClass}',sFilter:'$sFilter',bSearchMode:$JSSearchMode, json: function() { return $sWizHelperJSON; } }});
		//}
		/*************** EOF Edited By Nilesh New for regardless department show callers *******************/
...
EOF
	...
	public function GetClassSelectionForm(WebPage $oPage)
	{
		...
		$oPage->add('</select>');
		$oPage->add('&nbsp; <button type="submit" class="action" style="margin-top:7px;"><span>' . Dict::S('UI:Button:Ok') . '</span></button></nobr></p>');
	...
	public function GetObjectCreationForm(WebPage $oPage, $oCurrObject, $aPrefillFormParam)
	{
		...
		$titlename = Dict::Format('UI:CreationTitle_Class', MetaModel::GetName($this->sTargetClass));
		if($titlename == 'Creation of a new Organization'){
			$titlename = "Creation of a new Department";
		}
		if($titlename == 'Creación de Organización'){
			$titlename = "Creación de Departamento";
		}
		...
		$oPage->add("<h1>".MetaModel::GetClassIcon($this->sTargetClass)."&nbsp;".$titlename."</h1>\n");
...
//		$oPage->add_ready_script("\$('#ac_create_$this->iId').dialog({ width: $(window).width()*0.8, height: 'auto', autoOpen: false, modal: true, title: '$sDialogTitle'});\n");
	...
	public function DisplayHierarchy(WebPage $oPage, $sFilter, $currValue, $oObj)
	{
		...
		$hierarchyTitle =Dict::Format('UI:HierarchyOf_Class', MetaModel::GetName($this->sTargetClass));		
		if($hierarchyTitle=='Hierarchy of Organization'){
			$hierarchyTitle = 'Hierarchy of Department';
		}
		if($hierarchyTitle=='Jerarquía de Organización'){
			$hierarchyTitle = 'Jerarquía de Departamento';
		}
		$sDialogTitle = addslashes($hierarchyTitle);
