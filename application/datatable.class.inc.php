...
EOF;
	...
	protected function GetToolkitMenu(WebPage $oPage, $aExtraParams)
	{
		if (!$oPage->IsPrintableVersion())
		{
			...
			$sHtml = '<div class="nt3_popup toolkit_menu" id="tk_'.$this->iListId.'"><ul><li><img src="../images/toolkit_menu.png?t='.utils::GetCacheBusterTimestamp().'"><ul>';
	
			$oMenuItem1 = new JSPopupMenuItem('nt3::ConfigureList', $sMenuTitle, "$('#datatable_dlg_".$this->iListId."').dialog('open');");
			$aActions = array(
...
print_r($aColumns);*/
	...
	public function GetHTMLTable(WebPage $oPage, $aColumns, $sSelectMode, $iPageSize, $bViewLink, $aExtraParams)
	{
		...
		/***** Edited by Nilesh ****/
		/*if($aAttribs['key_Change']['label']=='Change'){*/
		if(isset($aAttribs['key_Change']['label'])){
			$aAttribs['key_Change']['label'] = 'Ticket Number';
		}
		if(isset($aAttribs['key_Problem']['label'])){
			$aAttribs['key_Problem']['label'] = 'Ticket Number';
		}
		if(isset($aAttribs['key_Incident']['label'])){
			$aAttribs['key_Incident']['label'] = 'Ticket Number';
		}
		/***** End ****/
		/****** (Renaming)Edited By Priya*****/

		if(isset($aAttribs['org_id_Incident']['label'])){
			$aAttribs['org_id_Incident']['label'] = 'Area';
		}
		if(isset($aAttribs['org_id_Incident']['label'])){
			$aAttribs['org_id_Incident']['label'] = 'Area';
		}
		...
		if(isset($aAttribs['caller_id_Incident']['label'])){
			$aAttribs['caller_id_Incident']['label'] = 'Reported By';
		}
		if(isset($aAttribs['caller_id_Incident']['label'])){
			$aAttribs['caller_id_Incident']['label'] = 'Reported By';
		}
		...
		if(isset($aAttribs['origin_Incident']['label'])){
			$aAttribs['origin_Incident']['label'] = 'Via';
		}
		if(isset($aAttribs['origin_Incident']['label'])){
			$aAttribs['origin_Incident']['label'] = 'Via';
		}
...
EOF
		...
		//$oPage->add_ready_script('$(\'table .listResults tr th_heading_id_2\').html("Element sub-class");');
		//$oPage->add_ready_script('$(\'table .listResults tr th_heading_id_3\').html("Function Element");');
		///$oPage->add_ready_script('$("table .listResults thead tr th #th_heading_id_2").html("Element sub-class")');
		//$oPage->add_ready_script('$("table .listResults thead tr th #th_heading_id_3").html("Function Element")');
		if ($sFakeSortList != '')
		{
