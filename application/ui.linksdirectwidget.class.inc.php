...
class UILinksWidgetDirect
{
	...
	protected function DisplayEditInPlace(WebPage $oPage, $oValue, $aArgs = array(), $sFormPrefix, $oCurrentObj, $aButtons = array('create', 'delete'))
	{
		...
		$addReal = MetaModel::GetName($this->sLinkedClass);
		if($addReal=='Organization'){
			$addReal = 'Department';
		}
		if($addReal=='Organización'){
			$addReal = 'Departamento';
		}
		...
		$aLabels = array(
			...
			'add' => Dict::Format('UI:AddAnExisting_Class', $addReal),
	...
	public function GetObjectsSelectionDlg($oPage, $oCurrentObj, $aAlreadyLinked)
	{
		...
		$sHtml .= "<button type=\"button\" class=\"cancel popupcnclfun\">".Dict::S('UI:Button:Cancel')."</button>&nbsp;&nbsp;<button type=\"button\" class=\"ok popupokfun\" disabled=\"disabled\">".Dict::S('UI:Button:Add')."</button>";
		$sHtml .= "</div>\n";
		$sHtml .= "</form>\n";
		$oPage->add($sHtml);
	}
