...
abstract class Dashboard
{
	...
	public function RenderProperties($oPage)
	{
		...
		$oForm->RenderAsPropertySheet($oPage, false, '.nt3-dashboard');	
...
<<<EOF
	$('#select_layout input').click( function() {
		...
		$('.nt3-dashboard').runtimedashboard('option', {layout_class: sLayoutClass});
	} );
	$('#row_attr_dashboard_title').property_field('option', {parent_selector: '.nt3-dashboard', auto_apply: false, 'do_apply': function() {
			var sTitle = $('#attr_dashboard_title').val();
			$('.nt3-dashboard').runtimedashboard('option', {title: sTitle});
			return true;
		}
	});
	$('#row_attr_auto_reload').property_field('option', {parent_selector: '.nt3-dashboard', auto_apply: true, 'do_apply': function() {
			var bAutoReload = $('#attr_auto_reload').is(':checked');
			$('.nt3-dashboard').runtimedashboard('option', {auto_reload: bAutoReload});
			return true;
		}
	});
	$('#row_attr_auto_reload_sec').property_field('option', {parent_selector: '.nt3-dashboard', auto_apply: true, 'do_apply': function() {
			var iAutoReloadSec = $('#attr_auto_reload_sec').val();
			$('.nt3-dashboard').runtimedashboard('option', {auto_reload_sec: iAutoReloadSec});
			return true;
		}
	});
EOF
...
$('#dashboard_editor').dialog({
	...
	{ text: "$sOkButtonLabel", click: function() {
		var oDashboard = $('.nt3-dashboard').data('nt3Runtimedashboard');
