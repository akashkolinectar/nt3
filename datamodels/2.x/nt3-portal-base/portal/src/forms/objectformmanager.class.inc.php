...
class ObjectFormManager extends FormManager
{
	...
	/**
	 * Creates an instance of \Combodo\nt3\Portal\Form\ObjectFormManager from JSON data that must contain at least :
...
//		die();
						...
						if ($oAttDef->GetEditClass() === 'CustomFields')
						{
							...
							if ($oSubForm->HasField('user_data'))
							{
								...
								foreach ($oUserDataForm->GetFields() as $oCustomField)
								{
									...
									if (in_array(get_class($oCustomField), array('Combodo\\nt3\\Form\\Field\\SelectObjectField')))
									{
										if ($this->oApp !== null)
										{
	...
	public function OnCancel($aArgs = null)
	{
		...
		// TODO : This has to be refactored when the function from nt3-attachent has been migrated into the core
	 ...
	 * It is inspired from nt3-attachments/main.attachments.php / UpdateAttachments()
	 *
