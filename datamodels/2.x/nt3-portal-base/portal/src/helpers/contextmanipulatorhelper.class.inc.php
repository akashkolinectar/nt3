...
class ContextManipulatorHelper
{
	...
	public function Init(DOMNodeList $oNodes)
	{
		...
		foreach ($oNodes as $oRuleNode)
		{
			...
			$bNeedsSource = false;
			// Note : preset and retrofit are no longer plurals as it should match as much as possible nt3ObjectCopier specs. We use plurals only in the xml for the collection tags
			$aRule = array(
				'source_oql' => null,
				'dest_class' => null,
				'preset' => array(),
			...
			foreach ($oRuleNode->childNodes as $oSubNode)
			{
				...
				switch ($sSubNodeName)
				{
					...
					case 'retrofits':
						...
						{
							// Note : Caution, the index of $aRule is now $oActionNode->nodeName instead of $sSubNodeName, as we want to match nt3ObjectCopier specs like told previously
			...
			foreach ($aData['rules'] as $sId)
			{
				...
				if ($aRule['source_oql'] !== null)
				{
					...
					while ($oSourceObject = $oSet->Fetch())
					{
						...
						// Old code : nt3ObjectCopier::PrepareObject($aRule, $oObject, $oSourceObject);
						// Presets
				...
				{
					// Changing behaviour to remove usage of ObjectCopier as its now being integrated in the core
					// Old code : nt3ObjectCopier::PrepareObject($aRule, $oObject, $oObject);
					// Presets
					if (isset($aRule['preset']) && !empty($aRule['preset']))
					{
