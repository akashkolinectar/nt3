...
class ApplicationHelper
{
	...
	public static function RegisterRoutes(Application $oApp)
	{
		...
		{
			if (is_subclass_of($sPHPClass, 'Combodo\\nt3\\Portal\\Router\\AbstractRouter'))
			{
				try
				{
	...
	public static function RegisterTwigExtensions(Twig_Environment &$oTwigEnv)
	{
		...
		// Filter to add nt3version to an url
		$oTwigEnv->addFilter(new Twig_SimpleFilter('add_nt3_version', function ($sUrl) {
			if (strpos($sUrl, '?') === false)
			{
				$sUrl = $sUrl."?nt3version=".nt3_VERSION;
			}
			else
			{
				$sUrl = $sUrl."&nt3version=".nt3_VERSION;
		...
		if (1 || !$oApp['debug'])
		{
			$oApp->error(function (Exception $oException, Request $oRequest) use ($oApp) {
				...
				else
				{
					...
					$oResponse = $oApp['twig']->render('nt3-portal-base/portal/src/views/errors/layout.html.twig',
						$aData);
				}
		...
		try
		{
			...
			$aPortalConf = array(
				'properties' => array(
					...
					'logo' => (file_exists(MODULESROOT.'branding/portal-logo.png')) ? utils::GetAbsoluteUrlModulesRoot().'branding/portal-logo.png' : '../images/logo-nt3-dark-bg.svg',
					'themes' => array(
						'bootstrap' => 'nt3-portal-base/portal/web/css/bootstrap-theme-combodo.scss',
						'portal' => 'nt3-portal-base/portal/web/css/portal.scss',
						'others' => array(),
					),
					'templates' => array(
						'layout' => 'nt3-portal-base/portal/src/views/layout.html.twig',
						'home' => 'nt3-portal-base/portal/src/views/home/layout.html.twig'
					),
					'urlmaker_class' => null,
					'triggers_query' => null,
					'attachments' => array(
	 ...
	 *
	 * @param \Combodo\nt3\Portal\Brick\AbstractBrick $oBrick
	 *
	 * @throws \Exception
	 */
