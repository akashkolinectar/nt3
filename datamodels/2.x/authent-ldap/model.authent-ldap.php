...
class UserLDAP extends UserInternal
{
	...
	public function CheckCredentials($sPassword)
	{
		...
		if ($hDS === false)
		{
			$this->LogMessage("ldap_authentication: can not connect to the LDAP server '$sLDAPHost' (port: $iLDAPPort). Check the configuration file config-nt3.php.");
			return false;
		}
		...
		if ($bind = @ldap_bind($hDS, $sDefaultLDAPUser, $sDefaultLDAPPwd))
		{
			...
			switch($iCountEntries)
			{
				...
				$this->LogMessage("ldap_authentication: several (".ldap_count_entries($hDS, $hSearchResult).") entries match the query '$sQuery', base_dn = '$sBaseDN', check that the query defined in config-nt3.php is specific enough.");
			}
			return false;
		}
		else
		{
			...
			$this->LogMessage("ldap_authentication: can not bind to the LDAP server '$sLDAPHost' (port: $iLDAPPort), user='$sDefaultLDAPUser', pwd='$sDefaultLDAPPwd'. Error: '".ldap_error($hDS)."'. Check the configuration file config-nt3.php.");
			return false;
		}
	}
