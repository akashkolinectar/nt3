...
class ApplicationInstaller
{
	...
	protected static function DoCompile($aSelectedModules, $sSourceDir, $sExtensionDir, $sTargetDir, $sEnvironment, $bUseSymbolicLinks = false)
	{
		...
		// Special case to patch a ugly patch in nt3-config-mgmt
		$sFileToPatch = $sTargetPath.'/nt3-config-mgmt-1.0.0/model.nt3-config-mgmt.php';
		if (file_exists($sFileToPatch))
		{
			$sContent = file_get_contents($sFileToPatch);
			
			$sContent = str_replace("require_once(APPROOT.'modules/nt3-welcome-itil/model.nt3-welcome-itil.php');", "//\n// The line below is no longer needed in NT3 2.0 -- patched by the setup program\n// require_once(APPROOT.'modules/nt3-welcome-itil/model.nt3-welcome-itil.php');", $sContent);
	...
	)
	{
		...
		// Set a DBProperty with a unique ID to identify this instance of NT3
		$sUUID = DBProperty::GetProperty('database_uuid', '');
		if ($sUUID === '')
		{
			$sUUID = utils::CreateUUID('database');
			DBProperty::SetProperty('database_uuid', $sUUID, 'Installation/upgrade of '.nt3_APPLICATION, 'Unique ID of this '.nt3_APPLICATION.' Database');
		}
	...
	/**
	 * Helper function to create and administrator account for NT3
	...
	protected static function DoCreateConfig(
		...
		$sConfigFile = APPCONF.(($sTargetEnvironment == '') ? 'production' : $sTargetEnvironment).'/'.nt3_CONFIG_FILE;
