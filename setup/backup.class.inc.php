...
interface BackupArchive
{
	 ...
	 * Usage: $oArchive->extractDirTo('/var/www/html/nt3/data', '/production-modules/')
	 *
	 * @param string $sDestinationDir
...
if (class_exists('ZipArchive')) // The setup must be able to start even if the "zip" extension is not loaded
{
	...
	class ZipArchiveEx extends ZipArchive implements BackupArchive
	{
		...
		/**
		 * Extract a whole directory from the archive.
		 * Usage: $oZip->extractDirTo('/var/www/html/nt3/data', '/production-modules/')
		 *
		 * @param string $sDestinationDir
	...
	class DBBackup
	{
		 ...
		 *
		 * @param sNameSpec string Name and path, eventually containing nt3 placeholders + time formatting specs
		 */
		 ...
		 * Create a normalized backup name, depending on the current date/time and Database
		 *
		 * @param string sNameSpec Name and path, eventually containing nt3 placeholders + time formatting specs
		 */
		...
		public function CreateZip($sZipFile, $sSourceConfigFile = null)
		{
			...
			$sDataFile = tempnam(SetupUtils::GetTmpDir(), 'nt3-');
		...
		protected function PrepareFilesToBackup($sSourceConfigFile, $sTmpFolder)
		{
			...
			{
				$sFile = $sTmpFolder.'/config-nt3.php';
			...
			$sDataFile = $sTmpFolder.'/nt3-dump.sql';
			$this->DoBackup($sDataFile);
			$aRet[] = $sDataFile;
		...
		public function DoBackup($sBackupFileName)
		{
			...
			if ($this->sDBSubName != '')
			{
				// This instance of NT3 uses a prefix for the tables, so there may be other tables in the database
