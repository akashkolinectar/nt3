...
class SetupUtils
{
	...
	// -- versions that will be the minimum in next NT3 major release (warning if not met)
	const PHP_NEXT_MIN_VERSION = ''; // no new PHP requirement for now in NT3 2.6
	const MYSQL_NEXT_MIN_VERSION = ''; // no new MySQL requirement for now in NT3 2.6
	...
	static function CheckPhpAndExtensions()
	{
		...
		if (extension_loaded('apc'))
		{
			...
			$aResult[] = new CheckResult(CheckResult::INFO, "APC detected (version $sAPCVersion). The APC cache will be used to speed-up ".nt3_APPLICATION.".");
		}
	...
	private static function CheckPhpVersion(&$aResult)
	{
		...
		if (version_compare($sPhpVersion, self::PHP_MIN_VERSION, '>='))
		{
			$aResult[] = new CheckResult(CheckResult::INFO,
				"The current PHP Version (".$sPhpVersion.") is greater than the minimum version required to run ".nt3_APPLICATION.", which is (".self::PHP_MIN_VERSION.")");
			...
			if (!empty($sPhpNextMinVersion))
			{
				...
				{
					$aResult[] = new CheckResult(CheckResult::INFO,
						"The current PHP Version (".$sPhpVersion.") is greater than the minimum version required to run next ".nt3_APPLICATION." release, which is (".self::PHP_NEXT_MIN_VERSION.")");
				}
				else
				{
					$aResult[] = new CheckResult(CheckResult::WARNING,
						"The current PHP Version (".$sPhpVersion.") is lower than the minimum version required to run next ".nt3_APPLICATION." release, which is (".self::PHP_NEXT_MIN_VERSION.")");
				}
			}
		...
		else
		{
			$aResult[] = new CheckResult(CheckResult::ERROR,
				"Error: The current PHP Version (".$sPhpVersion.") is lower than the minimum version required to run ".nt3_APPLICATION.", which is (".self::PHP_MIN_VERSION.")");
		}
	}
	...
	static function GetPreviousInstance($sDir)
	{
		...
		if (file_exists($sDir.'/config-nt3.php'))
		{
			$sSourceDir = $sDir;
			$sSourceEnvironment = '';
			$sConfigFile = $sDir.'/config-nt3.php';
			$aResult['found'] = true;
		}
		else if (file_exists($sDir.'/conf/production/config-nt3.php'))
		{
			$sSourceDir = $sDir;
			$sSourceEnvironment = 'production';
			$sConfigFile = $sDir.'/conf/production/config-nt3.php';
			$aResult['found'] = true;
		}
		...
		if ($aResult['found'])
		{
			...
			$aResult = array(
				...
				'mysql_bindir' => $oPrevConf->GetModuleSetting('nt3-backup', 'mysql_bindir', ''),
			);
		}
	...
	static function DisplayDBParameters(
		...
		$oPage->add('<tr style="display:none"><td colspan="3" style="background-color: #f9e0df; padding: 1em; border: 1px solid #950303; color: #950303;">Before configuring MySQL with TLS encryption, read the documentation <a href="https://wiki.opennt3.org/doku.php?id=2_5_0:install:php_and_mysql_tls" target="_blank">on Combodo\'s Wiki</a></td></tr>');
...
EOF
	...
	private static function CheckDbServerVersion(&$aResult, $oDBSource)
	{
		...
		if (version_compare($sDBVersion, self::MYSQL_MIN_VERSION, '>='))
		{
			...
			if (!empty($sMySqlNextMinVersion))
			{
				...
				{
					$aResult['checks'][] = new CheckResult(CheckResult::INFO,
						"Current MySQL version ($sDBVersion), greater than minimum required version for next ".nt3_APPLICATION." release (".self::MYSQL_NEXT_MIN_VERSION.")");
				}
				else
				{
					$aResult['checks'][] = new CheckResult(CheckResult::WARNING,
						"Warning : Current MySQL version is $sDBVersion, minimum required version for next ".nt3_APPLICATION." release will be ".self::MYSQL_NEXT_MIN_VERSION);
				}
			}
     ...
     * @param string $sBaseDir Path to the root directory of NT3
...
//echo "CheckDirAgainstManifest($sBaseDir, $sSourceDir ...)\n"; 
		...
		foreach($aManifest as $aFileInfo)
		{
			...
			if ($sDir == '.')
			{
				// Hmm... the file seems located at the root of NT3
				$sDir = '';
			}
			if ($sDir == $sSourceDir)
			{
...
//		echo "<pre>Comparison of ".dirname($sBaseDir)."/portal:\n".print_r($aResults, true)."</pre>";
	...
	public static function CheckWritableDirs($aWritableDirs)
	{
		...
		foreach($aWritableDirs as $sDir)
		{
			...
			{
				$aNonWritableDirs[APPROOT.$sDir] = new CheckResult(CheckResult::ERROR, nt3_APPLICATION." needs the directory <b>'".APPROOT.$sDir."'</b> to be writable. However <i>file</i> named <b>'".APPROOT.$sDir."'</b> already exists.");
			}
			else if (!is_dir($sFullPath) && !is_writable(APPROOT))
			{
				$aNonWritableDirs[APPROOT.$sDir] = new CheckResult(CheckResult::ERROR, nt3_APPLICATION." needs the directory <b>'".APPROOT.$sDir."'</b> to be writable. The directory <b>'".APPROOT.$sDir."'</b> does not exist and '".APPROOT."' is not writable, the application cannot create the directory '$sDir' inside it.");
			}				
		}
...
class SetupInfo
{
	 ...
	 * @param string $sModuleId The identifier of the module (without the version number. Example: nt3-config-mgmt)
