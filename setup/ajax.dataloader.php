	...
	switch($sOperation)
	{
		...
		if (is_subclass_of($sClass, 'WizardStep'))
		{
			...
			if (file_exists($sConfigFile) && !is_writable($sConfigFile) && $oStep->RequiresWritableConfig())
			{
				...
				$oPage->p("The wizard cannot modify the configuration file for you. If you want to upgrade ".nt3_APPLICATION.", make sure that the file '<b>".realpath($sConfigFile)."</b>' can be modified by the web server.");
				$oPage->output();
			}
			else
			{
