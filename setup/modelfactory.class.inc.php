...
require_once(APPROOT.'setup/nt3designformat.class.inc.php');
...
class MFDictModule extends MFModule
{
	...
	public function GetDictionaryFiles()
	{
		...
		if ($hDir = opendir($this->sRootDir))
		{
			...
			{
				$aMatches = array();
				if (preg_match("/^.*dictionary\\.nt3.*.php$/i", $sFile, $aMatches)) // Dictionary files are named like <Lang>.dict.<ModuleName>.php
				{
					$aDictionaries[] = $this->sRootDir.'/'.$sFile;
				}
			}
...
class ModelFactory
{
	...
	public function __construct($aRootDirs, $aRootNodeExtensions = array())
	{
		...
		$this->oRoot = $this->oDOMDocument->CreateElement('nt3_design');
		$this->oRoot->setAttribute('xmlns:xsi', "http://www.w3.org/2001/XMLSchema-instance");
		$this->oRoot->setAttribute('version', nt3_DESIGN_LATEST_VERSION);
	...
	public function LoadDelta($oSourceNode, $oTargetParentNode)
	{
		...
		if (($oSourceNode->tagName == 'class') && ($oSourceNode->parentNode->tagName == 'classes') && ($oSourceNode->parentNode->parentNode->tagName == 'nt3_design'))
		{
			...
			if (($sDeltaSpec == 'define') || ($sDeltaSpec == 'force'))
			{
				...
				$oTargetParentNode = $oTarget->GetNodeById('/nt3_design/classes//class', $sParentId)->item(0);
	
				if (!$oTargetParentNode)
				{
					echo "Dumping target doc - looking for '$sParentId'<br/>\n";
					$this->oDOMDocument->firstChild->Dump();
					$sPath = MFDocument::Getnt3NodePath($oSourceNode);
			...
			{
				$oTargetNode = $oTarget->GetNodeById('/nt3_design/classes//class', $oSourceNode->getAttribute('id'))->item(0);
				if (!$oTargetNode)
				{
	...
	public function LoadModule(MFModule $oModule, $aLanguages = array())
	{
		try
		{
			...
			foreach($aDataModels as $sXmlFile)
			{
				...
				$oNodeList = $oXPath->query('/nt3_design/classes//class');
				...
				$oNodeList = $oXPath->query('/nt3_design/constants/constant');
				foreach($oNodeList as $oNode)
				{
					if ($oNode->getAttribute('_created_in') == '')
					{
				...
				$oNodeList = $oXPath->query('/nt3_design/menus/menu');
				foreach($oNodeList as $oNode)
				{
					if ($oNode->getAttribute('_created_in') == '')
					{
				...
				$oUserRightsNode = $oXPath->query('/nt3_design/user_rights')->item(0);
				if ($oUserRightsNode)
				{
				...
				$oAlteredNodes = $oXPath->query('/nt3_design//*[@_delta]');
...
EOF
	...
	{
		return $this->GetNodes("/nt3_design/constants/constant[@_created_in='$sModuleName']");
	}
	...
	{
		return $this->GetNodes("/nt3_design/classes//class[@_created_in='$sModuleName']");
	}
	...
	{
		return $this->GetNodes("/nt3_design/classes//class");
	}
	...
	{
		return $this->GetNodes("/nt3_design/classes/class/class[class]");
	}
	...
	{
		$oClassNode = $this->GetNodes("/nt3_design/classes//class[@id='$sClassName']")->item(0);
		return $oClassNode;
	}
	...
	public function GetDeltaDocument($aNodesToIgnore = array(), $aAttributes = null)
	{
		...
		$aMovedClasses = array();
		foreach($this->GetNodes("/nt3_design/classes//class/class[@_alteration='removed']", null, false) as $oNode)
		{
			$sId = $oNode->getAttribute('id');
			if ($this->GetNodes("/nt3_design/classes//class/class[@id='$sId']/properties", null, false)->length > 0)
			{
		...
		$oNodesToClean = $oDelta->GetNodes('/nt3_design//*[@_altered_in]');
...
EOF;
			...
			$oRoot = $this->GetNodes('//nt3_design')->item(0);
			//$oRoot->Dump();
 ...
 * @package ModelFactory
 */
class MFElement extends Combodo\nt3\DesignElement
{
	/**
	 * Extracts some nodes from the DOM
	 *
	...
	public function AddChildNode(MFElement $oNode)
	{
		...
		if ($oExisting)
		{
			...
			{
				$sPath = MFDocument::Getnt3NodePath($oNode);
	...
	public function RedefineChildNode(MFElement $oNode, $sSearchId = null)
	{
		...
		if (!$oExisting)
		{
			$sPath = MFDocument::Getnt3NodePath($this)."/".$oNode->tagName.(empty($sSearchId) ? '' : "[$sSearchId]");
			$iLine = $oNode->getLineNo();
	...
	public function IsClassNode()
	{
		...
		{
			if (($this->parentNode->tagName == 'classes') && ($this->parentNode->parentNode->tagName == 'nt3_design') ) // Beware: classes/class also exists in the group definition
			{
				return true;
			}
	...
	public function MergeInto($oContainer, $sSearchId, $bMustExist, $bIfExists = false)
	{
		...
		if ($oTargetNode)
		{
			if ($oTargetNode->getAttribute('_alteration') == 'removed')
			{
				if ($bMustExist)
				{
					throw new Exception(MFDocument::Getnt3NodePath($this).' at line '.$this->getLineNo().": could not be found (marked as deleted)");
				}
		...
		else
		{
			if ($bMustExist)
			{
				...
				$oContainer->Dump();
				throw new Exception(MFDocument::Getnt3NodePath($this).' at line '.$this->getLineNo().": could not be found");
			}
			if (!$bIfExists)
			{
				// Beware: ImportNode(xxx, false) DOES NOT copy the node's attribute on *some* PHP versions (<5.2.17)
 ...
 * @package ModelFactory
 */
class MFDocument extends \Combodo\nt3\DesignDocument
{
	/**
	...
	public function saveXML(DOMNode $node = null, $options = 0)
	{
		...
		if (!$oRootNode)
		{
			$oRootNode = $this->createElement('nt3_design'); // make sure that the document is not empty
			$oRootNode->setAttribute('xmlns:xsi', "http://www.w3.org/2001/XMLSchema-instance");
			$oRootNode->setAttribute('version', nt3_DESIGN_LATEST_VERSION);
