...
class DOMFormatException extends Exception
{
    ...
    public function __construct($message, $code = null, $previous = null, DOMNode $node = null)
    {
        if($node !== null)
        {
            $message .= ' ('.MFDocument::Getnt3NodePath($node).' at line '.$node->getLineNo().')';
        }
...
class MFCompiler
{
	...
	public function Compile($sTargetDir, $oP = null, $bUseSymbolicLinks = false, $bSkipTempDir = false)
	{
		...
		else
		{
			...
			$sTempTargetDir = tempnam(SetupUtils::GetTmpDir(), 'nt3-');
	...
	protected function DoCompile($sTempTargetDir, $sFinalTargetDir, $oP = null, $bUseSymbolicLinks = false)
	{
		...
		// Then model.nt3-profiles-itil would not exist... resulting in an error after the compilation (and the actual product of the compiler would never be included
		...
		{
			if ($oModule->GetName() == 'nt3-profiles-itil')
			{
				$sUserRightsModule = 'nt3-profiles-itil';
				break;
			}
		}
...
EOF;
		...
		// Compile the portals
		$oPortalsNode = $this->oFactory->GetNodes('/nt3_design/portals')->item(0);
		$this->CompilePortals($oPortalsNode, $sTempTargetDir, $sFinalTargetDir);

		// Create module design XML files
		$oModuleDesignsNode = $this->oFactory->GetNodes('/nt3_design/module_designs')->item(0);
		$this->CompileModuleDesigns($oModuleDesignsNode, $sTempTargetDir, $sFinalTargetDir);

		// Compile the XML parameters
		$oParametersNode = $this->oFactory->GetNodes('/nt3_design/module_parameters')->item(0);
...
EOF;
	...
	protected function CompileFiles($oNode, $sTempTargetDir, $sFinalTargetDir, $sRelativePath)
	{
		...
		foreach ($oFileRefs as $oFileRef)
		{
			...
			if ($sFileId !== '')
			{
				$oNodes = $this->oFactory->GetNodes("/nt3_design/files/file[@id='$sFileId']");
				if ($oNodes->length == 0)
				{
	...
	{
		$oSnippets = $this->oFactory->GetNodes('/nt3_design/snippets/snippet');
