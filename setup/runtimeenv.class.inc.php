...
class RunTimeEnvironment
{
	...
	public function InitDataModel($oConfig, $bModelOnly = true, $bUseCache = false)
	{
		...
		{
			$this->oExtensionsMap = new nt3ExtensionsMap($this->sTargetEnv);
		}
	}
	...
	public function AnalyzeInstallation($oConfig, $modulesPath, $bAbortOnMissingDependency = false, $aModulesToLoad = null)
	{
		$aRes = array(
			ROOT_MODULE => array(
				'version_db' => '',
				'name_db' => '',
				'version_code' => nt3_VERSION.'.'.nt3_REVISION,
				'name_code' => nt3_APPLICATION,
			)
		);
		...
		foreach($aModules as $sModuleId => $aModuleInfo)
		{
			...
			$sModuleAppVersion = $aModuleInfo['nt3_version'];
		...
		foreach ($aSelectInstall as $aInstall)
		{
			...
			if ($sModuleVersion == '')
			{
				// Though the version cannot be empty in NT3 2.0, it used to be possible
	...
	public function WriteConfigFileSafe($oConfig)
	{
		...
		$sTargetConfigFile = APPCONF.$this->sTargetEnv.'/'.nt3_CONFIG_FILE;
	 ...
	 * @return boolean
	 */
	protected function IsExtensionSelected(nt3Extension $oExtension)
	{
		return ($oExtension->sSource == nt3Extension::SOURCE_REMOTE);
	}
	...
	protected function GetMFModulesToCompile($sSourceEnv, $sSourceDir)
	{
		...
		//
		$oSourceConfig = new Config(APPCONF.$sSourceEnv.'/'.nt3_CONFIG_FILE);
		...
		$this->oExtensionsMap = new nt3ExtensionsMap($this->sTargetEnv, true, $aExtraDirs);
		...
		//
		$oDictModule = new MFDictModule('dictionaries', 'NT3 Dictionaries', APPROOT.'dictionaries');
		...
		foreach($aModules as $oModule)
		{
			...
			$bIsExtra = $this->oExtensionsMap->ModuleIsChosenAsPartOfAnExtension($sModule, nt3Extension::SOURCE_REMOTE);
	...
	public function CreateDatabaseStructure(Config $oConfig, $sMode)
	{
		...
		if ($sMode == 'install')
		{
			...
			else
			{
				if (strlen($oConfig->Get('db_subname')) > 0)
				{
					throw new Exception("Error: found NT3 tables into the database '".$oConfig->Get('db_name')."' (prefix: '".$oConfig->Get('db_subname')."'). Please, try selecting another database instance or specify another prefix to prevent conflicting table names.");
				}
				else
				{
					throw new Exception("Error: found NT3 tables into the database '".$oConfig->Get('db_name')."'. Please, try selecting another database instance or specify a prefix to prevent conflicting table names.");
				}
			}
		}
		else
		{
			...
			else
			{
				if (strlen($oConfig->Get('db_subname')) > 0)
				{
					throw new Exception("Error: No previous instance of NT3 found into the database '".$oConfig->Get('db_name')."' (prefix: '".$oConfig->Get('db_subname')."'). Please, try selecting another database instance.");
				}
				else
				{
					throw new Exception("Error: No previous instance of NT3 found into the database '".$oConfig->Get('db_name')."'. Please, try selecting another database instance.");
				}
			}
		}
		return true;
	...
	public function RecordInstallation(Config $oConfig, $sDataModelVersion, $aSelectedModuleCodes, $aSelectedExtensionCodes, $sShortComment = null)
	{
		...
		$iMainnt3Record = $oInstallRec->DBInsertNoReload();
		
		// Record main installation
		$oInstallRec = new ModuleInstallation();
		$oInstallRec->Set('name', nt3_APPLICATION);
		$oInstallRec->Set('version', nt3_VERSION.'.'.nt3_REVISION);
		...
		foreach($aSelectedModuleCodes as $sModuleId)
		{
			...
			$oInstallRec->Set('parent_id', $iMainnt3Record);
			$oInstallRec->Set('installed', $iInstallationTime);
	...
	public function Commit()
	{
		if ($this->sFinalEnv != $this->sTargetEnv)
		{
			...
			//
			$sTargetConfig = APPCONF.$this->sTargetEnv.'/config-nt3.php';
			$sFinalConfig = APPCONF.$this->sFinalEnv.'/config-nt3.php';
