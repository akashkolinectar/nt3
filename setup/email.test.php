...
/**
 * Wizard to configure and initialize the NT3 application
 */
...
$oP = new SetupPage('NT3 email test utility');
...
function CheckEmailSetting($oP)
{
	...
	{
		case 'PHPMail':
		$oP->info("NT3 is configured to use PHP's <a style=\"background:transparent;padding:0;color:#000;text-decoration:underline;\" href=\"http://www.php.net/manual/en/function.mail.php\" target=\"_blank\">mail</a> function to send emails.");
		...
		case 'SMTP':
		$oP->info("NT3 is configured to use the <b>SMTP</b> transport.");
		...
		$oP->info("SMTP configuration (from config-nt3.php): host: $sHost, port: $sPort, user: $sDisplayUserName, password: $sDisplayPassword, encryption: $sDisplayEncryption.");
		if (($sHost == 'localhost') && ($sPort == '25') && ($sUserName == '') && ($sPassword == '') )
		{
			$oP->warning("The default settings may not be suitable for your environment. You may want to ajust these values by editing NT3's configuration file (".APPROOT."conf/production/config-nt3.php).");
		}
		break;
		
		case 'Null':
		$oP->warning("NT3 is configured to use the <b>Null</b> transport: emails sending will have no effect.");
		$bRet = false;
		break;
		
		case 'LogFile':
		$oP->warning("NT3 is configured to use the <b>LogFile</b> transport: emails will <em>not</em> be sent but logged to the file: '".APPROOT."/log/mail.log'.");
		$bRet = true;
		break;
	...
	if ($bConfigAsync)
	{
		$oP->warning("NT3 is configured to send emails <em>asynchronously</em>. Make sure that cron.php is scheduled to run in the background, otherwise regular emails will <em>not</em> be sent. For the purpose of this test, the email will be sent <em>synchronously</em>.");
	}
...
function DisplayStep2(SetupPage $oP, $sFrom, $sTo)
{
	...
	$oP->add("<h1>NT3 configuration wizard</h1>\n");
	...
	$oEmail->SetSubject("Test NT3");
	$oEmail->SetBody("<p>Hello,</p><p>The email function is now working fine.</p><p>You may now be able to use the notification function.</p><p>NT3</p>");
