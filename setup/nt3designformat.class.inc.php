 ...
 * $oFormat = new nt3DesignFormat($oDocument);
 * if ($oFormat->Convert())
 * {
...
define('nt3_DESIGN_LATEST_VERSION', '1.5'); // NT3 >= 2.5.0
 
class nt3DesignFormat
{
	 ...
	 * @return string
	 */
	public static function Getnt3NodePath($oNode)
	{
		if ($oNode instanceof DOMDocument) return '';
	
		$sId = $oNode->getAttribute('id');
		$sNodeDesc = ($sId != '') ? $oNode->nodeName.'['.$sId.']' : $oNode->nodeName;
		return self::Getnt3NodePath($oNode->parentNode).'/'.$sNodeDesc;
	}	 	
	 ...
	 */
	public function CheckConvert($sTargetVersion = nt3_DESIGN_LATEST_VERSION, $oFactory = null)
	{
		// Clone the document
	 ...
	 */
	public function Convert($sTargetVersion = nt3_DESIGN_LATEST_VERSION, $oFactory = null)
	{
		$this->aLog = array();
		$this->bStatus = true;
		...
		$oNodeList = $oXPath->query('/nt3_design');
		if ($oNodeList->length == 0)
		{
			// Hmm, not an NT3 Data Model file...
			$this->LogError('File format, no root <nt3_design> tag found');
		}
		else
		{
	...
	protected function From10To11($oFactory)
	{
		...
		$oXPath = new DOMXPath($this->oDocument);
		$oNodeList = $oXPath->query('/nt3_design/classes//class/lifecycle/states/state/transitions/transition/stimulus');
		...
		// which don't already have one
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeStopWatch']/thresholds/threshold/percent");
		foreach ($oNodeList as $oNode)
		{
			$oNode->parentNode->SetAttribute('id', $oNode->textContent);
			$this->DeleteNode($oNode);
		...
		// which don't already have one
		$oNodeList = $oXPath->query('/nt3_design/user_rights/profiles/profile/groups/group/actions/action');
		foreach ($oNodeList as $oNode)
		{
		...
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeEnum']/values/value");
		foreach ($oNodeList as $oNode)
		{
			if ($oNode->getAttribute('id') == '')
			{
	...
	protected function From11To10($oFactory)
	{
		...
		$oXPath = new DOMXPath($this->oDocument);
		$oNodeList = $oXPath->query('/nt3_design/classes//class/lifecycle/states/state/transitions/transition[@id]');
		foreach ($oNodeList as $oNode)
		{
			if ($oXPath->query('descendant-or-self::*[@_delta or @_rename_from]', $oNode)->length > 0)
			{
				$this->LogError('Alterations have been defined under the node: '.self::Getnt3NodePath($oNode));
			}
		...
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeStopWatch']/thresholds/threshold[@id]");
		foreach ($oNodeList as $oNode)
		{
			if ($oXPath->query('descendant-or-self::*[@_delta or @_rename_from]', $oNode)->length > 0)
			{
		...
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeEnum']/values/value[@id]");
		foreach ($oNodeList as $oNode)
		{
			if ($oXPath->query('descendant-or-self::*[@_delta or @_rename_from]', $oNode)->length > 0)
			{
	...
	protected function From12To11($oFactory)
	{
		...
		//
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeObjectKey']");
		foreach ($oNodeList as $oNode)
		{
			$oNode->setAttribute('xsi:type', 'AttributeInteger');
			// The property class_attcode is left there (doing no harm)
			$this->LogWarning('The attribute '.self::Getnt3NodePath($oNode).' has been degraded into an integer attribute. Any OQL query using this attribute will fail.');
		}

		// Remove Redundancy settings attributes (no redundancy could be defined in the previous format)
		//
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeRedundancySettings']");
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The attribute '.self::Getnt3NodePath($oNode).' is of no use and must be removed.');
			$this->DeleteNode($oNode);
		}

		// Later: transform the relations into code (iif defined as an SQL query)
		$oNodeList = $oXPath->query('/nt3_design/classes//class/relations');
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The relations defined in '.self::Getnt3NodePath($oNode).' will be lost.');
			$this->DeleteNode($oNode);
		}

		$oNodeList = $oXPath->query('/nt3_design/portal');
		foreach ($oNodeList as $oNode)
		{
		...
		$oNodeList = $oXPath->query('/nt3_design/module_parameters');
		foreach ($oNodeList as $oNode)
		{
		...
		$oNodeList = $oXPath->query('/nt3_design/snippets');
		foreach ($oNodeList as $oNode)
		{
	...
	protected function From13To12($oFactory)
	{
		...
		$oNodeList = $oXPath->query('/nt3_design/module_designs/module_design');
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The module design defined in '.self::Getnt3NodePath($oNode).' will be lost.');
			$this->DeleteNode($oNode);
		}

		// Remove MetaEnum attributes
		//
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeMetaEnum']");
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The attribute '.self::Getnt3NodePath($oNode).' is irrelevant and must be removed.');
			$this->DeleteNode($oNode);
		}

		// Remove CustomFields attributes
		//
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeCustomFields']");
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The attribute '.self::Getnt3NodePath($oNode).' is irrelevant and must be removed.');
			$this->DeleteNode($oNode);
		...
		//
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@xsi:type='AttributeImage']");
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The attribute '.self::Getnt3NodePath($oNode).' is irrelevant and must be removed.');
			$this->DeleteNode($oNode);
		...
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('The flag _delta="if_exists" on '.self::Getnt3NodePath($oNode).' is irrelevant and must be replaced by _delta="must_exist".');
	...
	protected function From14To13($oFactory)
	{
		...
		//
		$oNodeList = $oXPath->query("/nt3_design/classes//class/fields/field[@_delta='force']");
		$iCount = 0;
		foreach ($oNodeList as $oNode)
		{
        ...
        //
        $oNodeList = $oXPath->query("/nt3_design/classes//class/lifecycle/states/state/transitions/transition/flags");
        $this->LogWarning('Before removing flags nodes');
        foreach ($oNodeList as $oNode)
        {
            $this->LogWarning('Attribute flags '.self::Getnt3NodePath($oNode).' is irrelevant on transition and must be removed.');
            $this->DeleteNode($oNode);
        }
	}
	...
	protected function From15To14($oFactory)
	{
		...
		//
		$sPath = "/nt3_design/menus/menu[@xsi:type!='MenuGroup' and @xsi:type!='TemplateMenuNode']";
		$oNodeList = $oXPath->query("$sPath/enable_class | $sPath/enable_action | $sPath/enable_permission | $sPath/enable_stimulus");
		foreach ($oNodeList as $oNode)
		{
			$this->LogWarning('Node '.self::Getnt3NodePath($oNode).' is irrelevant in this version, it will be ignored. Use enable_admin_only instead.');
		}
	}
