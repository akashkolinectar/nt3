...
/**
 * First step of the NT3 Installation Wizard: Welcome screen
 */
class WizStepWelcome extends WizardStep
{
	...
	{
		return 'Welcome to '.nt3_APPLICATION.' version '.nt3_VERSION;
	}
...
EOF
		...
		$oPage->add('<h1>'.nt3_APPLICATION.' Installation Wizard</h1>');
...
/**
 * Second step of the NT3 Installation Wizard: Install or Upgrade
 */
class WizStepInstallOrUpgrade extends WizardStep
{
	...
	public function Display(WebPage $oPage)
	{
		...
		if ($sInstallMode == '')
		{
			$sDBBackupPath = APPROOT.'data/'.nt3_APPLICATION.strftime('-backup-%Y-%m-%d');
			$bDBBackup = true;
        ...
        $oPage->p('<input id="radio_install" type="radio" name="install_mode" value="install" '.$sChecked.'/><label for="radio_install">&nbsp;Install a new '.nt3_APPLICATION.'</label>');
		$sChecked = ($sInstallMode == 'upgrade') ? ' checked ' : '';
        $oPage->p('<input id="radio_update" type="radio" name="install_mode" value="upgrade" '.$sChecked.'/><label for="radio_update">&nbsp;Upgrade an existing '.nt3_APPLICATION.' instance</label>');
        ...
        $oPage->add('<tr><td colspan="2"><input id="db_backup" type="checkbox" name="db_backup" '.$sChecked.$sDisabled.' value="1"/><label for="db_backup">&nbsp;Backup the '.nt3_APPLICATION.' database before upgrading</label></td></tr>');
...
EOF
		...
		{
			throw(new Exception('No previous version of '.nt3_APPLICATION.' found in the supplied database. The upgrade cannot continue.'));
		}
		else if (strcasecmp($aInstalledInfo['product_name'], nt3_APPLICATION) != 0)
		{
			$oPage->p("<b>Warning: The installed products seem different. Are you sure the you want to upgrade {$aInstalledInfo['product_name']} with ".nt3_APPLICATION."?</b>");			
		}
		
		$sInstalledVersion = $aInstalledInfo['product_version'];
		$sInstalledDataModelVersion = $aInstalledInfo['datamodel_version'];
				
		$oPage->add("<h2>Information about the upgrade from version $sInstalledVersion to ".nt3_VERSION.'.'.nt3_REVISION."</h2>");
		
		if ($sInstalledVersion == (nt3_VERSION.'.'.nt3_REVISION))
		{
		...
		if ($sInstalledDataModelVersion == '$nt3_VERSION$.$WCREV$')
		{
		...
		if ($sCompatibleDMDir === false)
		{
			...
			$this->bCanMoveForward = false;
			$oPage->p("The current version of ".nt3_APPLICATION." (".nt3_VERSION.'.'.nt3_REVISION.") does not seem to be compatible with the installed version ($sInstalledVersion).");
		...
		else
		{
			...
			if (($aChanges !== false) && ( (count($aChanges['added']) > 0) || (count($aChanges['removed']) > 0) || (count($aChanges['modified']) > 0)) )
			{
				...
				$oPage->p("<img src=\"../images/error.png\"/>&nbsp;Some modifications were detected between the ".nt3_APPLICATION." version in '$sPreviousVersionDir' and a genuine $sInstalledVersion version.");
                ...
                $oPage->p('<input id="radio_upgrade_keep" type="radio" name="upgrade_type" value="keep-previous" '.$sChecked.$sDisabled.'/><label for="radio_upgrade_keep">&nbsp;Preserve the modifications of the installed version (the dasboards inside '.nt3_APPLICATION.' may not be editable).</label>');
...
EOF
			...
			if ($oMutex->IsLocked())
			{
				$oPage->p("<img src=\"../images/error.png\"/>&nbsp;An NT3 CRON process is being executed on the target database. It is highly recommended to stop any NT3 CRON process prior to running the setup program.");
			}
		}
	}
...
class WizStepLicense extends WizardStep
{
    ...
    public function Display(WebPage $oPage)
    {
		...
		$oPage->add('<h2>Licenses agreements for the components of '.nt3_APPLICATION.'</h2>');
		...
		$oPage->add('<fieldset>');
		$oPage->add('<legend>Components of '.nt3_APPLICATION.'</legend>');
		$oPage->add('<ul>');
        $index = 0;
...
class WizStepModulesChoice extends WizardStep
{
	...
	public function __construct(WizardController $oWizard, $sCurrentState)
	{
		...
		$this->oExtensionsMap = new nt3ExtensionsMap();
		$sPreviousSourceDir = $this->oWizard->GetParameter('previous_version_dir', '');
		$sConfigPath = null;
		if (($sPreviousSourceDir !== '') && is_readable($sPreviousSourceDir.'/conf/production/config-nt3.php'))
		{
			$sConfigPath = $sPreviousSourceDir.'/conf/production/config-nt3.php';
		}
...
EOF
	...
	protected function GetStepInfo($idx = null)
	{
		...
		if (@file_exists($this->GetSourceFilePath()))
		{
			...
			{
				if (($oExtension->sSource !== nt3Extension::SOURCE_WIZARD) && ($oExtension->bVisible) && (count($oExtension->aMissingDependencies) == 0))
				{
					$aStepDefinition['options'][] = array(
							...
							'mandatory' => $oExtension->bMandatory || ($oExtension->sSource === nt3Extension::SOURCE_REMOTE),
	...
	protected function GetExtensionSourceLabel($sSource)
	{
		switch($sSource)
		{
			...
			$sResult = (nt3_APPLICATION == 'nt3') ? 'nt3-Hub' : 'ITSM-Designer';
			break;
...
EOF
		...
		$sDestination = nt3_APPLICATION.(($sMode == 'install') ? ' version '.nt3_VERSION.' is about to be installed ' : ' is about to be upgraded ');
		...
		{
			$oPage->add('<li>Prefix for the '.nt3_APPLICATION.' tables: '.$aInstallParams['database']['prefix'].'</li>');
		}
		else
		{
			$oPage->add('<li>Prefix for the '.nt3_APPLICATION.' tables: none</li>');
		}
		$oPage->add('</ul></div>');
...
class WizStepDone extends WizardStep
{
	...
	public function Display(WebPage $oPage)
	{
		...
		if (count($aManualSteps) > 0)
		{
			...
			$oPage->add("<h2>Congratulations for installing ".nt3_APPLICATION."</h2>");
		}
		else
		{
			$oPage->add("<h2>Congratulations for installing ".nt3_APPLICATION."</h2>");
		...
		$oPage->add("<td><a style=\"background:transparent;padding:0;\" title=\"Free: Register your NT3 version.\" href=\"".NT3_HOME."/register?product=nt3&version=".urlencode(nt3_VERSION." revision ".nt3_REVISION)."\" target=\"_blank\"><img style=\"border:0\" src=\"../images/setup-register.gif\"/></td></a>");
		$oPage->add("<td><a style=\"background:transparent;padding:0;\" title=\"Get Professional Support from Combodo\" href=\"".NT3_HOME."/nt3support\" target=\"_blank\"><img style=\"border:0\" src=\"../images/setup-support.gif\"/></td></a>");
		$oPage->add("<td><a style=\"background:transparent;padding:0;\" title=\"Get Professional Training from Combodo\" href=\"".NT3_HOME."/nt3training\" target=\"_blank\"><img style=\"border:0\" src=\"../images/setup-training.gif\"/></td></a>");
		$oPage->add('</tr></table>');
		...
		$sIframeUrl = $oConfig->GetModuleSetting('nt3-hub-connector', 'setup_url', '');
		...
		if ($sIframeUrl != '')
		{
			...
			$oPage->add_script("window.addEventListener('message', function(event) {
				if (event.data === 'nt3hub_load_completed')
				{
		...
		$sForm .= "<p style=\"text-align:center;width:100%\"><button id=\"enter_nt3\" type=\"submit\">Enter ".nt3_APPLICATION."</button></p>";
		$sForm .= '</form>';
		...
		$sImgUrl = 'http://'.NT3_HOME.'/stats/?'.implode('&', $aSafeParams);
