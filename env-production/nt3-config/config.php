...
$oP = new nt3WebPage(Dict::S('config-edit-title'));
...
try
{
	...
	{
		$oP->add("<div class=\"header_message message_info\">Sorry, NT3 is in <b>demonstration mode</b>: the configuration file cannot be edited.</div>");
	}
	else if (MetaModel::GetModuleSetting('nt3-config', 'config_editor', '') == 'disabled')
	{
		$oP->add("<div class=\"header_message message_info\">NT3 interactive edition of the configuration as been disabled. See <tt>'config_editor' => 'disabled'</tt> in the configuration file.</div>");
	}
	else
	{
		$sConfigFile = APPROOT.'conf/'.utils::GetCurrentEnvironment().'/config-nt3.php';
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        ...
                        $sTmpFile = tempnam(SetupUtils::GetTmpDir(), 'nt3-cfg-');
                        ...
                        // If you are really concerned about an NT3 administrator crafting some malicious
