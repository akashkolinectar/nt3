...
class AttachmentPlugIn implements iApplicationUIExtension, iApplicationObjectExtension
{
	...
	{
		return MetaModel::GetModuleSetting('nt3-attachments', 'position', 'relations');
	}
	...
	public function DisplayAttachments($oObject, WebPage $oPage, $bEditMode = false)
	{
		...
		$sMaxWidth = MetaModel::GetModuleSetting('nt3-attachment', 'inline_image_max_width', '450px');
		$oPage->add_style(
<<<EOF
.attachment {
...
<<< EOF
    $('#file').fileupload({
		url: GetAbsoluteUrlModulesRoot()+'nt3-attachments/ajax.attachment.php',
...
EOF
		...
		$iMaxWidth = MetaModel::GetModuleSetting('nt3-attachments', 'preview_max_width', 290);
		$oPage->add_ready_script(
<<<EOF
	$(document).tooltip({
		items: '.attachment a',
...
EOF
	...
	protected static function UpdateAttachments($oObject, $oChange = null)
	{
		...
		{
			// Workaround to an issue in NT3 < 2.0
	...
	public static function GetFileIcon($sFileName)
	{
		...
		return 'env-'.utils::GetCurrentEnvironment()."/nt3-attachments/icons/$sIcon";
	}
    ...
    public static function IsReadonlyState(DBObject $oObject, $sState, $sGUI = self::ENUM_GUI_ALL)
    {
        ...
        $bReadonly = false;
        $sClass = get_class($oObject);
        $aReadonlyStatus = MetaModel::GetModuleSetting('nt3-attachments', 'readonly_states', $aParamDefaultValue);
