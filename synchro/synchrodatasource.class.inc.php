...
EOF
	...
	protected function GetTriggersDefinition()
	{
		...
		$sTriggerUpdate .= "      IF @nt3user is null THEN";
	...
	public function SendNotification($sSubject, $sBody)
	{
		...
		$sSubject = 'NT3 Data Sync - '.$this->GetName().' - '.$sSubject;
...
class SynchroReplica extends DBObject implements iDisplay
{
	...
	public static function Init()
	{
		...
		MetaModel::Init_AddAttribute(new AttributeObjectKey("dest_id", array("allowed_values"=>null, "class_attcode"=>"dest_class", "sql"=>"dest_id", "is_null_allowed"=>true, "depends_on"=>array())));
...
class SynchroExecution
{
	...
	*/	
	 ...
	 * and the NT3 objects. If the lastFullLoadStartDate is NOT specified then the full_load_periodicity
	...
	public function Process()
	{
		...
		$oMutex = new nt3Mutex('synchro_process_'.$this->m_oDataSource->GetKey());
		try
		{
			$oMutex->Lock();
			$this->DoSynchronize();
	...
	protected function DoSynchronize()
	{
		...
		if ($iMaxChunkSize > 0)
		{
			...
			do
			{
				...
				list ($iRes, $aOut) = utils::Execnt3Script('synchro/priv_sync_chunk.php', $aArguments);
