...
define('nt3_APPLICATION', 'NT3');
define('nt3_APPLICATION_SHORT', 'NT3');
define('nt3_VERSION', '2.5.0');
define('nt3_REVISION', '3935');
define('nt3_BUILD_DATE', '2018-07-12 09:51:53');
...
// so we switched to utf8mb4 in NT3 2.5, adding dependency to MySQL 5.5.3
// The config params db_character_set and db_collation were introduced as a temporary workaround and removed in NT3 2.5
...
define('NT3_HOME', 'https://nt3.nectarinfotel.com'); // Created by Vidya
...
class Config
{
	...
	protected $m_aSettings = array(
		...
		'db_character_set' => array( // @deprecated to remove in 2.7 ? #1001 utf8mb4 switch
			'type' => 'string',
			'description' => 'Deprecated since NT3 2.5 : now using utf8mb4',
		...
		'db_collation' => array( // @deprecated to remove in 2.7 ? #1001 utf8mb4 switch
			'type' => 'string',
			'description' => 'Deprecated since NT3 2.5 : now using utf8mb4_unicode_ci',
			'default' => 'DEPRECATED_2.5',
			'value' => '',
			'source_of_value' => '',
			'show_in_conf_sample' => false,
		...
		'access_message' => array(
			...
			'default' => 'NT3 is temporarily frozen, please wait... (the admin team)',
			'value' => '',
			'source_of_value' => '',
			'show_in_conf_sample' => true,
		),
		...
		'cas_profile_pattern' => array(
			'type' => 'string',
			'description' => 'A regular expression pattern to extract the name of the NT3 profile from the name of an LDAP/CAS group',
			// examples... not used (nor 'description')
			'default' => '/^cn=([^,]+),/',
			'value' => '/^cn=([^,]+),/',
			'source_of_value' => '',
		...
		'cas_default_profiles' => array(
			'type' => 'string',
			'description' => 'A semi-colon separated list of NT3 Profiles to use when creating a new user if no profile is retrieved from CAS',
			// examples... not used (nor 'description')
			'default' => 'Portal user',
			'value' => 'Portal user',
			'source_of_value' => '',
		...
		'min_reload_interval' => array(
			...
			'default' => 5, // In NT3 2.0.3, this was the hardcoded value
			'value' => '',
			'source_of_value' => '',
			'show_in_conf_sample' => false,
		),
		'relations_max_depth' => array(
			...
			'default' => 20, // In NT3 2.0.3, this was the hardcoded value
			'value' => '',
			'source_of_value' => '',
			'show_in_conf_sample' => false,
		),
	 ...
	 *     are related to a whole domain, and a domain can host multiple NT3)
	 *     Beware: do not expose server side information to the client !
	 */
	public function Getnt3Instanceid()
	{
	 ...
	 *              If set, any attempt to connect to an NT3 page with http:// will be redirected
	 *              to https://
	 */
	 ...
	 *             unless you want to import a database from another NT3 instance, in which case you must use
	...
	public function __construct($sConfigFile = null, $bLoadConfig = true)
	{
		...
		$this->m_aAddons = array(
			// Default AddOn, always present can be moved to an official NT3 Module later if needed
	...
	public function WriteToFile($sFileName = '')
	{
		...
		if ($hFile !== false)
		{
			...
			fwrite($hFile, "\n/**\n");
			fwrite($hFile, " *\n");
			fwrite($hFile, " * Configuration file, generated by the ".nt3_APPLICATION." configuration wizard\n");
			fwrite($hFile, " *\n");
			fwrite($hFile,
