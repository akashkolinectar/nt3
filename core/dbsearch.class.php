 ...
 * An object search
 * 
 * Note: in the ancient times of NT3, a search was named after DBObjectSearch.
...
abstract class DBSearch
{
	...
	static public function unserialize($sValue)
	{
		...
		/************* Edited by Nilesh for overview link ticket views **************/
		if(isset($_COOKIE['pre_days']) && isset($_COOKIE['new_days'])){
			$dd = $_COOKIE['new_days'];
			/*if(isset($aData[0])){
				$aData[0] = str_replace('20', $dd, $aData[0]);
			}*/

			if(isset($_GET['c']) && isset($aData[0])){
				if($_GET['c']['menu']=="Incident:Overview"){
					$aData[0] = str_replace("INTERVAL 60 DAY", "INTERVAL ".$dd." DAY", $aData[0], $count);
					$aData[0] = str_replace("INTERVAL 20 DAY", "INTERVAL ".$dd." DAY", $aData[0], $count);
					$aData[0] = str_replace("INTERVAL 14 DAY", "INTERVAL ".$dd." DAY", $aData[0], $count);
				}

				if($_GET['c']['menu']=="Problem:Overview"){
					/*$sSql = str_replace("GROUP BY", " AND (DATE_SUB(NOW(), INTERVAL ".$dd." DAY) < `Change_related_change_id_ntticket`.`start_date`) GROUP BY", $sSql);*/
				}

				if($_GET['c']['menu']=="Change:Overview"){
					$aData[0] = str_replace("INTERVAL 20 DAY", "INTERVAL ".$dd." DAY", $aData[0], $count);
					$aData[0] = str_replace("INTERVAL 7 DAY", "INTERVAL ".$dd." DAY", $aData[0], $count);
				}
			}
		}
		/************* Edited by Nilesh for overview link ticket views **************/
	...
	static public function FromOQL($sQuery, $aParams = null)
	{
		...
		if ($bOQLCacheEnabled)
		{
			...
			elseif (self::$m_bUseAPCCache)
			{
				...
				//
				$sAPCCacheId = 'nt3-'.MetaModel::GetEnvironmentId().'-dbsearch-cache-'.$sQueryId;
	...
	public function MakeGroupByQuery($aArgs, $aGroupByExpr, $bExcludeNullValues = false, $aSelectExpr = array(), $aOrderBy = array(), $iLimitCount = 0, $iLimitStart = 0)
	{
		...
		{
			if (!array_key_exists($sAlias, $aGroupByExpr) && !array_key_exists($sAlias, $aSelectExpr) && ($sAlias != '_nt3_count_'))
			{
				$aAllowedAliases = array_keys($aSelectExpr);
				$aAllowedAliases = array_merge($aAllowedAliases,  array_keys($aGroupByExpr));
				$aAllowedAliases[] = '_nt3_count_';
	...
	public function MakeSelectQuery($aOrderBy = array(), $aArgs = array(), $aAttToLoad = null, $aExtendedDataSpec = null, $iLimitCount = 0, $iLimitStart = 0, $bGetCount = false)
	{
		...
		try
		{
			...
			if ($sClassAlias == '_nt3_')
			{
				IssueLog::Info('SQL Query (_nt3_): '.$sRes);
			}
		}
