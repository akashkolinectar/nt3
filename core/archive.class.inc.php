...
/**
 * nt3Archive a class to manipulate (read/write) NT3 archives with their catalog
 * Each NT3 archive is a zip file that contains (at the root of the archive)
...
class nt3Archive
{
	...
	/**
	 * Construct an nt3Archive object
	 * @param $sArchivePath string The full path the archive file
	 * @param $iMode integrer Either nt3Archive::read for reading an existing archive or nt3Archive::create for creating a new one. Updating is not supported (yet)
	 */
	public function __construct($sArchivePath, $iMode = nt3Archive::read)
	{
	...
	*/
	  ...
	  */
	 public function ImportSql($sFileName, $sDatabase = 'nt3')
	 {
		...
		$sCommandLine = 'type "'.$sTempName.'" | "/nt3/MySQL Server 5.0/bin/mysql.exe" -u root '.$sDatabase;
	  ...
	  * @return boolean False in case of error, true otherwise
	  */
	 public function AddDatabaseDump($sTitle, $sDescription, $sFileName, $sDatabase = 'nt3', $aTables = array(), $bStructureOnly = true)
	 {
		$sTempName = tempnam("../tmp/", "sql");
		$sNoData = $bStructureOnly ? "--no-data" : "";
		$sCommandLine = "\"/nt3/MySQL Server 5.0/bin/mysqldump.exe\" --user=root --opt $sNoData --result-file=$sTempName $sDatabase ".implode(" ", $aTables);
...
$oArchive = new nt3Archive($sArchivePath, nt3Archive::read);
...
$oArchive = new nt3Archive($sArchivePath, nt3Archive::create);
...
$oArchive->SetTitle('First Archive !');
$oArchive->SetDescription('This is just a test. Does not contain a lot of useful data.');
$oArchive->AddDatabaseDump('Test', 'This is my first automatic dump', 'schema.sql', 'nt3', array('objects'));
$oArchive->WriteCatalog();
*/
?>
