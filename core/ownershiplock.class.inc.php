 ...
 */
class nt3OwnershipToken extends DBObject
{
...
class nt3OwnershipLock
{
	 ...
	 * @return multitype:boolean nt3OwnershipLock Ambigous <boolean, string, DBObjectSet>
	 */
	public static function AcquireLock($sObjClass, $iObjKey)
	{
		$oMutex = new nt3Mutex('lock_'.$sObjClass.'::'.$iObjKey);
		
		$oMutex->Lock();
		$oOwnershipLock = new nt3OwnershipLock($sObjClass, $iObjKey);
	...
	public static function KillLock($sObjClass, $iObjKey)
	{
		...
		$oMutex->Lock();
		$sOQL = "SELECT nt3OwnershipToken WHERE obj_class = :obj_class AND obj_key = :obj_key";
	...
	protected function __construct($sObjClass, $iObjKey)
	{
		...
		// IssueLog::Info("nt3OwnershipLock::__construct($sObjClass, $iObjKey) oToken::".($this->oToken ? $this->oToken->GetKey() : 'null'));
	}
	...
	protected function TakeOwnership($sToken = null)
	{
		if ($this->oToken == null)
		{
			$this->oToken = new nt3OwnershipToken();
	...
	{
		$sOQL = "SELECT nt3OwnershipToken WHERE last_seen < :last_seen_limit";
