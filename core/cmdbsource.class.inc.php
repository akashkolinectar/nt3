...
class CMDBSource
{
	...
	public static function QueryToArray($sSql)
	{
		...
		if(isset($_COOKIE['pre_days']) && isset($_COOKIE['new_days'])){
			...
			$pre_days = (int)$_COOKIE['pre_days'];
			if(isset($_GET['c'])){
				if($_GET['c']['menu']=="Incident:Overview"){
					$sSql = str_replace("INTERVAL 20 DAY", "INTERVAL ".$dd." DAY", $sSql, $count);
					$sSql = str_replace("INTERVAL 14 DAY", "INTERVAL ".$dd." DAY", $sSql, $count);
				}

				if($_GET['c']['menu']=="Problem:Overview"){
					$sSql = str_replace("GROUP BY", " AND (DATE_SUB(NOW(), INTERVAL ".$dd." DAY) < `Problem_ntticket`.`start_date`) GROUP BY", $sSql);
				}

				if($_GET['c']['menu']=="Change:Overview"){
					$sSql = str_replace("INTERVAL 20 DAY", "INTERVAL ".$dd." DAY", $sSql, $count);
					$sSql = str_replace("INTERVAL 7 DAY", "INTERVAL ".$dd." DAY", $sSql, $count);
				}
			}
			//$sSql = str_replace("INTERVAL 20 DAY", "INTERVAL ".$dd." DAY", $sSql, $count);
		}
