...
class DisplayableGroupNode extends DisplayableNode
{
	...
	public function GetTooltip($aContextDefs)
	{
		...
		$sHtml .= '<a href="#" onclick="$(\'.nt3-simple-graph\').simple_graph(\'show_group\', \'relation_group_'.$iGroupIdx.'\');">'.Dict::Format('UI:RelationGroupNumber_N', (1+$iGroupIdx))."</a>";
		$sHtml .= '<hr/>';
...
class DisplayableGraph extends SimpleGraph
{
	 ...
	 * @param string $sContextKey The key to fetch the queries in the configuration. Example: nt3-tickets/relation_context/UserRequest/impacts/down 
