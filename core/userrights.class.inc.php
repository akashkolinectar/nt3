...
{
	/**
	 * Called when no user is found in NT3 for the corresponding 'name'. This method
	 * can create/synchronize the User in NT3 with an external source (such as AD/LDAP) on the fly
	...
	/**
	 * Called after the user has been authenticated and found in NT3. This method can
...
class CAS_SelfRegister implements iSelfRegister
{
	...
	public static function CheckCredentialsAndCreateUser($sName, $sPassword, $sLoginMode, $sAuthentication)
	{
				...
				foreach($aMemberOf as $sGroupName)
				{
					...
					if ($bIsMember)
					{
						...
						if ($bCASUserSynchro)
						{
							...
							{
								phpCAS::log("User ".phpCAS::getUser()." cannot be created in NT3. Logging off...");
							}
						}
						else
						{
		...
		else
		{
			...
			phpCAS::log("User ".phpCAS::getUser().": missing user account in NT3 (or NT3 badly configured, Cf setting cas_memberof)");
			$bFound = false;
		}
	...
	protected static function CreateCASUser($sEmail, $aGroups)
	{
		...
		if ($oUser == null)
		{
			...
			{
				case 0:
				phpCAS::log("Error: found no contact with the email: '$sEmail'. Cannot create the user in NT3.");
				return false;
	...
	protected static function SetProfilesFromCAS($oUser, $aGroups)
	{
		...
		// Translate the CAS/LDAP group names into NT3 profile names
		$aProfiles = array();
		...
		foreach($aGroups as $sGroupName)
		{
			if (preg_match($sPattern, $sGroupName, $aMatches))
			{
				...
				{
					phpCAS::log("Warning: {$aMatches[1]} is not a valid NT3 profile (extracted from group name: '$sGroupName'). Ignored.");
				}
			}
			else
			{
				phpCAS::log("Info: The CAS group '$sGroupName' does not seem to match an NT3 pattern. Ignored.");
			}
		}
		if (count($aProfiles) == 0)
		{
			...
			foreach($aCASDefaultProfiles as $sDefaultProfileName)
			{
				...
				{
					phpCAS::log("Warning: the default profile {$sDefaultProfileName} is not a valid NT3 profile. Ignored.");
				}
			}
			
			if (count($aProfiles) == 0)
			{
				phpCAS::log("Error: The user '".$oUser->GetName()."' has no profiles in NT3, and therefore cannot be created.");
				return false;
			}
		}
