...
$(function()
{
	...
	$.widget( "nt3.datatable",
	{
		...
		_create: function()
		{
			...
			this.element.find('.nt3_popup > ul li').popupmenu();
		...
		onDlgOk: function()
		{
			...
			else
			{
				...
				var oColumns = $('#datatable_dlg_'+sListId).find(':nt3-fieldsorter').fieldsorter('get_params');
		...
		UpdateState: function( config )
		{
			...
			dlgElement.find(':nt3-fieldsorter').fieldsorter('option', { fields: this.options.oColumns });
		},
		_saveDlgState: function()
		{
			...
			var sId = new String(this.element.attr('id'));
			var sListId = sId.replace('datatable_', '');
			this.originalState.oFields = $('#datatable_dlg_'+sListId).find(':nt3-fieldsorter').fieldsorter('get_params');
		},
		_restoreDlgState: function()
		{
			...
			dlgElement.find(':nt3-fieldsorter').fieldsorter('option', { fields: this.originalState.oFields });
		},
