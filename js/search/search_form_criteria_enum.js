//NT3 Search form criteria enum
;
$(function()
{
	// the widget definition, where 'nt3' is the namespace,
	// 'search_form_criteria_enum' the widget name
	$.widget( 'nt3.search_form_criteria_enum', $.nt3.search_form_criteria,
	{
		// default options
		options:
		{
		...
		_bindEvents: function()
		{
			...
			// Add selected data
			this.element.on('nt3.search.criteria_enum.add_selected_values', function(oEvent, oData){
		...
		_prepareInOperator: function(oOpElem, sOpIdx, oOp)
		{
			...
			oFilterElem.find('.sff_reset').on('click', function(){
				oFilterElem.find('input')
					...
					.trigger('nt3.search.criteria_enum.autocomplete.submit');
			});
		...
		_prepareInOperatorWithAutocomplete: function(oOpElem, sOpIdx, oOp)
		{
			...
			oFilterElem.find('input').on('keyup nt3.search.criteria_enum.autocomplete.submit', function(oEvent){
				// TODO: Move on values with up and down arrow keys; select with space or enter.
		...
		_onACSearchFail: function(oResponse, sStatus)
		{
			if(sStatus !== 'abort')
			{
				...
				this._setACHint('=/');
				this.handler.triggerHandler('nt3.search.criteria.error_occured', sErrorMessage);
			}
		},
