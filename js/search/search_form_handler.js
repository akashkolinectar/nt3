...
$(function()
{
	...
	$.widget( 'nt3.search_form_handler',
	{
		...
		_create: function()
		{
			...
			//init others widgets :
			this.element.search_form_handler_history({'nt3_root_class':me.options.search.class_name});
		...
		_bindEvents: function()
		{
			...
			this.element.on('nt3.search.form.submit', function(oEvent, oData){
				me._onSubmit();
			});
			// - Search form has been reloaded by the page
			this.element.on('nt3.search.form.reloaded', function(){
			...
			// Criteria events
			this.element.on('nt3.search.criteria.value_changed', function(oEvent, oData){
				me._onCriteriaValueChanged(oData);
			});
			this.element.on('nt3.search.criteria.removed', function(oEvent, oData){
				me._onCriteriaRemoved(oData);
			});
			this.element.on('nt3.search.criteria.error_occured', function(oEvent, oData){
				me._onCriteriaErrorOccured(oData);
			});

			$('body').on('update_history.nt3', function(oEvent, oData) {
                ...
                if ($('.ui-dialog:visible :nt3-search_form_handler').length != 0)
                {
				...
				$('#nt3-breadcrumb')
					.breadcrumb('destroy')
					.breadcrumb({
					nt3_instance_id: oData['breadcrumb_instance_id'],
		...
		_updateSearch: function()
		{
			...
			this.elements.criterion_area.find('.sf_criterion_row').each(function(iIdx){
				...
				oCriterionRowElem.find('.search_form_criteria').each(function(){
					var oCriteriaData = $(this).triggerHandler('nt3.search.criteria.get_data');
		...
		_closeAllCriterion: function()
		{
			this.elements.criterion_area.find('.search_form_criteria.opened').each(function(){
				$(this).triggerHandler('nt3.search.criteria.close');
			});
		},
		...
		_getHtmlLiFromFieldRef: function(sFieldRef, aFieldCollectionsEligible) {
			...
			aFieldCollectionsEligible.forEach(function (sFieldCollection)
			{
				...
				/*******Edited by Priya ********/
				var titleElem = oField.label;
				if(titleElem.indexOf("CI") >= 0){
					titleElem = titleElem.replace('CI','Elements');
				}
				oFieldElem = $('<li></li>')
					.addClass('sfm_field')
					.attr('data-field-ref', sFieldRef)
					.append('<label ' + sFieldTitleAttr + '><input type="checkbox" value="' + sFieldRef + '" />' + titleElem + '</label>')
				/*oFieldElem = $('<li></li>')
					.addClass('sfm_field')
					.attr('data-field-ref', sFieldRef)
		...
		_addCriteria: function(oData, oCriterionGroupElem)
		{
			...
			// Instanciate widget
			$.nt3[sWidgetName](oData, oCriteriaElem);
		...
		{
			return 'search_form_criteria' + '_' + (($.nt3['search_form_criteria_'+sType] !== undefined) ? sType : 'raw');
		},
		// Criteria handlers
		...
		_onSubmitClick: function(oEvent)
		{
            ...
            {
                this.elements.criterion_area.find('.sf_criteria.opened').trigger('nt3.search.criteria.close');
