...
$(function()
{
	...
	$.widget( 'nt3.search_form_criteria',
	{
		...
		options:
		{
			...
			// Available operators. They can be extended or restricted by derivated widgets (see this._innt3erators() for more informations)
			'available_operators': {
				'=': {
		...
		_bindEvents: function()
		{
			...
			// Get criteria data
			this.element.on('nt3.search.criteria.get_data', function(oEvent, oData){
				return me._onGetData(oData);
			});

			// Get/SetCurrentValues callbacks handler
			this.element.on('nt3.search.criteria.get_current_values nt3.search.criteria.set_current_values', function(oEvent, oData){
			...
			// Close criteria
			this.element.on('nt3.search.criteria.close', function(){
				me._apply();
				me._close();
			});
		...
		_open: function()
		{
			...
			this.handler.triggerHandler('nt3.search.criteria.opening');
		...
		{
			this.element.remove();
			this.handler.triggerHandler('nt3.search.criteria.removed');
		},
		...
		_apply: function()
		{
			...
			this.handler.triggerHandler('nt3.search.criteria.value_changed');
		},
		...
		_prepareElement: function()
		{
			...
			/*************** Edited by Nilesh New for contact person regardless organization ********************/
			if(this.options.class=="Person" && this.options.field.code=='org_id'){
				this.options.is_removable = true;
			}
			/*************** EOF Edited by Nilesh New for contact person regardless organization *****************/
		...
		_setTitle: function(sTitle)
		{
			...
			/***Edited by priya****/
			if(sTitle.indexOf("I") >= 0){
				sTitle= sTitle.replace('CI','Element');
			}
