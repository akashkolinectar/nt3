//NT3 Search form criteria date_abstract
;
$(function()
{
	// the widget definition, where 'nt3' is the namespace,
	// 'search_form_criteria_date_abstract' the widget name
	$.widget( 'nt3.search_form_criteria_date_abstract', $.nt3.search_form_criteria,
	{
		// default options
		options:
		{
			...
			aInputsParam: [
				{
					...
					//  "getter_code":"from_time"		=> NT3 expect datetime to always provide time, so we must always use the datetimepicker in order to compute the value to be sent to NT3 event if whe are in the display mode "less" with datepicker visible
					//  "getter_suffix": " 00:00:00",	=> NT3 expect datetime to always provide time, so either we use the "getter_code" or we add a suffix to force the time
