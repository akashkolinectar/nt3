// jQuery UI style "widget" for editing an NT3 "dashboard"
...
$(function()
{
	...
	$.widget( "nt3.dashboard",
	{
		...
		_create: function()
		{
			...
			this.element
			.addClass('nt3-dashboard')
			.bind('add_dashlet.nt3_dashboard', function(event, oParams){
			...
			$('.nt3-dashboard').on('click', 'a', function(e) { e.preventDefault(); });
		...
		_get_state: function(oMergeInto)
		{
			...
			this.element.find('.layout_cell').each(function() {
				var aList = [];
				$(this).find(':nt3-dashlet').each(function() {
					var oDashlet = $(this).data('nt3Dashlet');
					if(oDashlet)
					{
		...
		{
			var iMaxId = 0;
			this.element.find(':nt3-dashlet').each(function() {
				var oDashlet = $(this).data('nt3Dashlet');
				if(oDashlet)
				{
					var oDashletParams = oDashlet.get_params();
			...
			this.element.find('table td').droppable({
				...
				drop: function(event, ui) {
					...
					if (oDropped.hasClass('dashlet'))
					{
						...
						var oDashlet = ui.draggable.data('nt3Dashlet');
					...
					else
					{
						...
						$('.nt3-dashboard').trigger('add_dashlet', {dashlet_class: sDashletClass, container: $(this), refresh: bRefresh });
					}
				}
			});	
		},
...
$(function()
{
	// the widget definition, where "nt3" is the namespace,
	// "dashboard" the widget name
	$.widget( "nt3.runtimedashboard", $.nt3.dashboard,
	{
		// default options
		options:
		{
		...
		_create: function()
		{
			...
			this.element
			.addClass('nt3-runtimedashboard')
			.bind('mark_as_modified.nt3-dashboard', function(){me.mark_as_modified();} );
		...
		{
			this.element
			.removeClass('nt3-runtimedashboard');
		...
		is_dirty: function()
		{
			if ($('#dashboard_editor .ui-layout-east .nt3-property-field-modified').size() > 0)
			{
				return true;
			}
			else
		...
		{
			$('#dashboard_editor .ui-layout-east .nt3-property-field-modified').trigger('apply_changes');
		},
		save: function()
		{
...
$(function()
{
	...
	$.widget( "nt3.dashboard_upload_dlg",
	{
		// default options
		options:
		{
		...
		_create: function()
		{
			...
			this.element
			.addClass('nt3-dashboard_upload_dlg')
			.dialog({
				modal: true,
				width: 500,
				height: 'auto',
		...
		{
			this.element
			.removeClass('nt3-dashboard_upload_dlg');		
		},
		// _setOptions is called with a hash of all options that are changing
		_setOptions: function()
		{
