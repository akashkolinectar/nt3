...
//NT3 Field set
//Used by nt3.form_handler and nt3.subform_field to list their fields
;
$(function()
{
	// the widget definition, where 'nt3' is the namespace,
	// 'field_set' the widget name
	$.widget( 'nt3.field_set',
	{
		// default options
		options:
		{
		...
		_onGetCurrentValues: function(oEvent, oData)
		{
			...
			for(var i in this.options.fields_list)
			{
				...
				else
				{
					if(window.console)
					{
						console.log('Field set : Cannot retrieve current value from field [' + this.options.field_identifier_attr + '="' + oField.id + '"][data-form-path="' + this.options.form_path + '"] as it seems to have no nt3.form_field widget attached.');
					}
				}
			}
