			...
			for(alias in this.options.fields)
			{
				for(k in this.options.fields[alias])
				{
					...
					if (f.label != '')
					{
						...
						var newlab = f.label;
						if(f.code=='caller_id'){
							var newlab = 'Reported by';
						}
						if(f.code=='org_id'){
							var newlab = 'Area';
						}
						if(f.code=='caller_id_friendlyname'){
							var newlab = 'Reported by (Friendly Name) ';
						}
						if(f.code=='caller_name'){
							var newlab = 'Reported by Name (Last Name)';
						}
						if(f.code=='origin'){
							var newlab = 'Via';
						}
						//var field = $('<li name="' + k + '" alias="' + f.alias + '" code="' + f.code + '"><input type="checkbox"' + sChecked + sDisabled + '/>&nbsp;<label>' + f.label + sSortOrder + '</label></li>');
						var field = $('<li name="' + k + '" alias="' + f.alias + '" code="' + f.code + '"><input type="checkbox"' + sChecked + sDisabled + '/>&nbsp;' + newlab + sSortOrder + '</li>');
