...
$(function()
{
	...
	$.widget( "nt3.simple_graph",
	{
		...
		_create_toolkit_menu: function()
		{
			...
			sHtml += '<div class="nt3_popup toolkit_menu graph" style="font-size: 12px;" id="'+sPopupMenuId+'"><ul><li><img src="../images/toolkit_menu.png"><ul>';
		        		...
		        		if( $('#relation_group_'+sGroupIndex).length > 0)
		        		{
			        		oResult = {
			        				callback: function(key, options) {
			        					var me = $('.nt3-simple-graph').data('nt3Simple_graph'); // need a live value
		...
		_on_export_as_attachment: function()
		{
			...
			$.post(sUrl, oParams, function(data) {
				...
				var sIcon = GetAbsoluteUrlModulesRoot()+'nt3-attachments/icons/pdf.png';
				if (jTab != null)
				{
					var re = /^([^(]+)\(([0-9]+)\)(.*)$/;
