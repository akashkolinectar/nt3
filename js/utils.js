// Some general purpose JS functions for the NT3 application
...
function ReloadBlock(divId, sStyle, sSerializedFilter, sExtraParams) {
	...
	var oDataTable = $('#'+divId+' :nt3-datatable');
...
function ReloadSearchForm(divId, sClassName, sBaseClass, sContext, sTableId, sExtraParams) {
	...
	$.post(GetAbsoluteUrlAppRoot()+'pages/ajax.render.php?'+sContext, oMap,
		function (data) {
			...
			oDiv.find('form').triggerHandler('nt3.search.form.reloaded');
		}
	);
}
 ...
 * depends on a global variable oUserPreferences created/filled by the nt3WebPage
...
function ExportStartExport() {
	...
	$(':nt3-tabularfieldsselector:visible').tabularfieldsselector('close_all_tooltips');
	$('#export-form').hide();
